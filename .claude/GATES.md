...
| Move Display gate (#592) | a Display media template that is host-relative, or a Display registrar that never sets the `image_url` key | a Display is rendered STANDALONE by wallets and explorers — they have no AresRPG origin for a relative path, and they key on `image_url` by that exact name |
