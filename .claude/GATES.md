...
| `scripts/check-move-field-limits.mjs` | struct-field counts over the `LimitsVerifier` cap | a Move struct grew past what the publish pipeline tolerates |
...
| BACKLOG OPEN-count gate | `BACKLOG.md`'s `## OPEN` section over 40 rows | SKIPs (not a fail) when `BACKLOG.md` isn't in the checkout — it's a board convention from a sibling repo, harmless here |
| Arch gate — semgrep (`scripts/semgrep-gate.sh`) | dataflow patterns: laundered store writes, fight-package effect purity | SKIPs green if the `semgrep` binary is absent — never fails the build for a missing tool |
| Arch gate — dependency-cruiser (`scripts/depcruise-gate.sh`) | fight-core import hermeticity, engine quarantine, any NEW import cycle | SKIPs green if `depcruise`/`bun` are unavailable |
