...
// ── ENV POLARITY, STATED (sponsor_state.mjs memoizes both at module load) ──
// The station wire is driven against the in-memory cap + reservation stash, and an in-memory counter is a
// per-process allowance — off localnet its absence refuses outright, before the station is ever reached
// (sponsor.store_required.test.js owns that polarity). So this file declares the one network where a store-less
// process may still sponsor. Nothing here resolves a PTB scope, so the release-derived allowlist that `localnet`
// leaves empty is never consulted: the scope rules are proven, allowlist and retired set together, by
// sponsor.allowlist_retired.test.js against the real testnet derivation.
