...
describe('init + the headless envelope proof (fake DSN, captured transport — never the real DSN)', () => {
  ...
  it('a forced error attempts an outbound envelope carrying area/action tags + context', async () => {
    ...
    const boom = new Error('forced sponsor error')
    report_error(boom, { area: 'sponsor', action: 'handle_post' })
    ...
    const [event] = events
    expect(event.exception.values[0].value).toBe('forced sponsor error')
    expect(event.environment).toBe('test')
    expect(event.release).toBe('test-sha')
    expect(event.tags.area).toBe('sponsor')
    expect(event.tags.action).toBe('handle_post')
  })
