...
test('one connection persists only the selected wallet and address, restores silently, and forgets on disconnect', async () => {
  ...
  await Bun.sleep(0)
  first.dispatch({ type: 'external_wallet/select', wallet_name: 'Test wallet', address: second })
  await Bun.sleep(0)
  ...
  await Bun.sleep(0)
  expect(calls.authorize).toEqual([false, true, true])
...
test('observer rearming preserves one session and reattaches provider invalidation', async () => {
  ...
  await Bun.sleep(0)
  runtime.dispatch({ type: 'external_wallet/select', wallet_name: 'Test wallet', address })
  await Bun.sleep(0)
