...
test('city compass markers persist outside cities with their bearing and distance', () => {
  ...
  expect(thebes).toMatchObject({ label: 'Thebes', distance: 1_488, x: 0.5, dungeon: false })
  expect(ruins?.distance).toBeGreaterThan(13_000)
})

test('the city marker carries the colocated dungeon indicator only while inside that city', () => {
  const [thebes] = city_compass_markers('nauvis', { x: 512, z: 0 }, 0)

  expect(thebes).toMatchObject({ id: 'thebes', distance: 0, dungeon: true })
})
