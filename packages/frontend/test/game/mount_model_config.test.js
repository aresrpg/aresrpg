...
describe('mount model transform config', () => {
  test('the live ln fast-travel dragon declares its facing flip and 3x scale in one model row', () => {
    const url = '/models/mobs/ln.glb'

    expect(fast_travel_dragon_file()).toBe('ln.glb')
    expect(mount_model_config(url)).toEqual({ scale: 3, facing: Math.PI, flight: true })
    expect(mount_model_scale(url, 0.4)).toBeCloseTo(1.2)
