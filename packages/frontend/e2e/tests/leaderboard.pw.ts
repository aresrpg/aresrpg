...
test('mobile contains long exact scores and hundreds of character badges without page overflow', async ({ page }) => {
  ...
  const badges = page.locator('[data-badge-row]').first()
  const visible = await badges.locator(':scope > span:not([data-badge-overflow])').count()
  await expect(badges.locator('[data-badge-overflow]')).toHaveText(`+${500 - visible}`)
...
test('badges fill one row and recalculate overflow when the available width changes', async ({ page }) => {
  ...
  await expect(row.locator('[data-badge-overflow]')).toHaveCount(0)
