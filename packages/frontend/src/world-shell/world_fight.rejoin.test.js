...
describe('boot resume vs a zombie world fight (REJOIN-SPAWN)', () => {
  test('an EXPIRED-placement fight whose liquidation is refused is NEVER adopted — the world session survives', async () => {
    ...
    // #1751/#2122/D48: the entry consents before it liquidates — this row measures the liquidation mechanics, so
    // it answers directly rather than through the entry's own gate (which, under D48, always answers 'rejoin').
    // The consent's own behaviour: test/world-shell/world_fight_resume_auto.test.js.
