...
describe('#932 — a refresh during PLACEMENT re-enters the fight', () => {
  ...
  test('a refused resume is never silent — it names the fight and the reason', async () => {
    ...
    try {
      // #1751/#2122/D48: the entry consents before it liquidates — this row measures the liquidation mechanics,
      // so it answers directly rather than through the entry's own gate (which, under D48, always answers
      // 'rejoin'). The consent's own behaviour: test/world-shell/world_fight_resume_auto.test.js.
