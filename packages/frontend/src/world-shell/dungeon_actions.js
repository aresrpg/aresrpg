 ...
 * A settle DERIVES NOTHING — it only releases — so both PTBs carry the character-keyed `fight_latch` and no
 * registry shard, and this door takes no derivation scope (sdk/src/fight.js `settle_and_take_ptb`/
 * `settle_open_world_ptb`). `world_id` is the RUN's world and stays: `dungeon::settle_run` still reads it.
 * @param {{ fight_id:string, run_pass_id?:string|null, world_id?:string|null,
 *   character_id:string, lost_group?:{world_id:string,zx:number,zy:number,index:number}|null }} args
 *   `lost_group` = the claimed mob group a DEFEATED open-world fight gives back (#609); null on a victory (the
 *   door refuses a won outcome).
