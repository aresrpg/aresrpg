...
/** ONE projection home: the world package's peer projections, bound to THIS app's store. `peers` is the
 *  presence fold's only roster — joining the p2p room IS the announcement, so there is no second registry
 *  to reconcile against. Identity prefers the chain record and falls back to the peer's self-declared row. */
