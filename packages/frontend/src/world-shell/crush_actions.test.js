...
const registry = spyOn(sdk_game, 'get_crush_registry').mockImplementation(() => async () => ({ by_key: new Map() }))
const compose = spyOn(sdk_game, 'crush_ptb').mockImplementation(() => (args) => {
  composed.push(args)
  return { fake: 'tx' }
})
const character_resolve = spyOn(kiosk_resolve, 'kiosk_for_character').mockResolvedValue({
  kiosk_id: '0xcharacter-kiosk',
  personal_kiosk_cap_id: '0xcharacter-cap',
})
const item_cap_resolve = spyOn(kiosk_resolve, 'cap_for_kiosk').mockResolvedValue(item_cap_id)
const submit = spyOn(tx_seam, 'run_tx_random').mockResolvedValue({ result: { digest: '0xdigest' } })
const refresh = spyOn(roster, 'load_roster').mockResolvedValue(undefined)

const spies = [template_map, template_type_map, registry, compose, character_resolve, item_cap_resolve, submit, refresh]
...
beforeEach(() => {
  ...
  reset_auth_mock({ address: '0xowner', wallet_name: 'zklogin' })
  set_expedition_sdk_mock(async () => fake_sdk)
  for (const test_spy of spies) test_spy.mockClear()
})
...
describe('crush_item kiosk resolution', () => {
  test("builds a sibling-kiosk item's PTB against the item's own kiosk and cap", async () => {
    ...
    expect(character_resolve).not.toHaveBeenCalled()
