...
//
//   WORLD BINDING: only the selected character's identity-matched RPC doc can feed discovery. Creation commits
//       membership atomically; later world changes are explicit actions in the manual world switcher.
...
import { derive_discovery_binding } from './world_travel_state.js'
...
export function DiscoveryPrompts() {
  ...
  const binding = derive_discovery_binding({
    character_id,
    documents: char_view.data,
  })
  const { world_id, document: character_doc } = binding
  ...
  // This 10s doc poll is the binding's long-term healer — publish every CONFIRMED read into the ONE binding
  // home (session_gate.js) so the scene gate tracks external changes too. Tagged source 'poll'
  // (world-travel binding-clobber fix): during the indexer catch-up window right after a manual travel,
  // this poll can still return the PRE-travel world — session_gate.js's stale-poll guard
  // discards that read instead of tearing the fresh write back down, and self-heals the instant a poll agrees.
  useEffect(() => {
    if (!character_id || char_view.loading || !binding.confirmed) return
    publish_world_binding(character_id, world_id, 'poll')
  }, [character_id, char_view.loading, binding.confirmed, world_id])
