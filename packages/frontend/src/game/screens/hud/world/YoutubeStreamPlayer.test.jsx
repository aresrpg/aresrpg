...
// renders nothing (so nothing ever reaches youtube.com), on the grid it renders the visible player region
// (YouTube's terms), the micro-label, the track line and the play/pause control. This repo's convention is
// renderToStaticMarkup with no jsdom (see PetFeedModal.test.jsx), which is exactly the right instrument here:
// it renders the markup WITHOUT running effects, so the gate is asserted with no API load and no channel
// handoff. The chain that feeds the gate is covered at both ends: the session's publish (source wiring, the
// embed_voxel_lifecycle.test.js idiom) and the reducer's fold.
//
...
const { YoutubeStreamPlayer } = await import('./YoutubeStreamPlayer.jsx')
...
function render(presentation) {
  ...
  return renderToStaticMarkup(createElement(I18nextProvider, { i18n }, createElement(YoutubeStreamPlayer, null)))
}
...
describe('the hack-mode YouTube radio', () => {
  ...
  test('on the grid it shows the micro-label, the track line and the play control', () => {
    ...
    expect(html).toContain('gw-ytp__lbl')
    expect(html).toContain('gw-ytp__track')
    expect(html).toContain('class="gw-ytp__btn"')
  ...
  test('the player region itself is rendered, never hidden (YouTube terms)', () => {
    const html = render('hackgrid')
    expect(html).toContain('gw-ytp__screen')
  ...
  test('every string it renders ships in the locales', () => {
    const source = readFileSync(new URL('./YoutubeStreamPlayer.jsx', import.meta.url), 'utf8')
