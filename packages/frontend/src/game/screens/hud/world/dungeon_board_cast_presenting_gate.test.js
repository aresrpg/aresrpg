...
describe('DungeonBoard reachable — gates on the SAME cast_presenting fact the MP-zone wash reads', () => {
  ...
  test('(B) reachable guards on fight?.cast_presenting (early-return AND the dep array)', async () => {
    ...
    const dep_start = body.indexOf('}, [')
    expect(dep_start).toBeGreaterThan(-1)
    const dep_end = body.indexOf('])', dep_start)
    expect(dep_end).toBeGreaterThan(dep_start)
    expect(body.slice(dep_start, dep_end)).toMatch(/cast_presenting/)
  })
})
