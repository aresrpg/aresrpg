...
import { validate_flush_casts } from './flush_validation.js'
...
export function DungeonBoard() {
  ...
  const flush_commit = async (draft_actions, background = false) => {
    ...
    // THE DECISION lives in flush_validation.js (pure, drivable); this edge only supplies the facts and emits the
    // effects. Trap cells committed THIS flush (survivors → chain truth) vs DROPPED trap drafts (their optimistic
    ...
    const committed = committed_state(fight_store.getState())
    const validated =
      me && dungeon && committed_caster_cell != null
        ? validate_flush_casts({
            cast_queue,
            evolved,
            occupied,
            obstacles,
            grid: dungeon_grid_of(dungeon),
            committed_caster_cell,
            caster_seat,
            caster_id: entity_id,
            my_spells,
            weapon_attack_id: WEAPON_ATTACK_ID,
            weapon_reach: me.weapon?.reach ?? WEAPON_ATTACK_RANGE[1],
            active_fighter,
            fallback_range: [cast_params.range_min, cast_params.range_max],
            committed_fighters: committed.fighters,
            mob_hp_of: (idx) => committed_mob_hp(fight_store.getState(), idx),
            spell_name_of: (entry, drafted_spell, is_weapon) =>
              is_weapon
                ? t('fight.weapon_attack')
                : t(`spells.spell_${entry.spell_key}`, { defaultValue: drafted_spell?.name ?? entry.spell_key }),
          })
        : { cast_actions: [], dropped: 0, cast_drops: [], trap_placed: [], trap_dropped: [], logs: [] }
    const { cast_actions, dropped, cast_drops, trap_placed, trap_dropped } = validated
    for (const { message, payload } of validated.logs) game_log('board', message, { ...payload, background })
