...
//
// DURATION renders mm:ss off `duration_ms` when the caller has one — 0 means no timestamp source exists yet
// upstream (never a fake 00:00).
//
 ...
 * @param {{ mine: boolean, spoils: { xp: number, tokens: number, loot: Array<{ item_type: string, name: string, amount: number }> }, items: any[], template_map: Map<string, any>, tt: ReturnType<typeof use_template_t>, pending: boolean, loot_units: number | null, t: (key: string, opts?: any) => string }} props
 */
      ...
      {spoils.loot.length > 0 ? (
        <div className="fe-tiles">
          {spoils.loot.slice(0, MAX_TILES).map((e, i) => (
            <LootTile key={e.item_type ?? i} entry={e} items={items} template_map={template_map} tt={tt} t={t} />
          ))}
        </div>
 ...
 *   spoils: { xp: number, tokens: number, loot: Array<{ item_type: string, name: string, amount: number }> } | null,
 *   items: any[],
 ...
 *   duration_ms?: number,   // wall-clock fight length; 0/absent → no timestamp source yet (never a fake 00:00)
...
export function FightReport({
  ...
  // Loot tooltips reuse the SAME chain-direct template map + description resolver inventory/findables use, so
  // a hover over a spoils tile shows the shared ItemDetailView (decoded stats + Display art). Cheap: the map
  // fetch is memoized app-wide; a defeat card (spoils=null → no tiles) simply never reads it.
  const tt = use_template_t()
  const [template_map, set_template_map] = useState(/** @type {Map<string, any>} */ (() => new Map()))
  useEffect(() => {
    let alive = true
    get_template_by_item_type_map().then((m) => alive && set_template_map(m))
    return () => {
      alive = false
    }
  }, [])
      ...
      >
        <div className="fe-head">
          ...
          <div className="fe-sub">
            ...
            {/* Show the fight's duration — mm:ss once the caller has a real
                timestamp delta; 0/absent stays silent rather than lie with a "0:00". A resume/poll-adopt
                observation (duration_partial) is a FLOOR, not the true length — "~" flags it honestly. */}
            {duration_ms > 0 ? ` · ${duration_partial ? '~' : ''}${format_mmss(duration_ms)}` : ''}
          </div>
