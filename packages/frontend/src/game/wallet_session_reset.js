...
export function reset_wallet_session(input = { type: 'wallet_session/reset' }) {
  ...
  // 4. P2P lobby — fully leave so the session id, my published state and the peer map all drop. REQUIRED:
  //    without it a re-join takes the idempotent no-op path and A's identity lingers as the room's identity.
  leave_lobby()
