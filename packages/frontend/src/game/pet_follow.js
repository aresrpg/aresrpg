...
// its owner's transform: it keeps its OWN world position and steers toward the owner with a dead zone. This is
...
//
// The spec (owner, #593): ① target = the owner's position; ② a 5-block DEAD ZONE — within it the pet does NOT
...
export const DEAD_ZONE_M = 5 // within ~5 blocks of the owner the pet does not chase (the spec's dead zone)
...
export const SNAP_LAND_M = 3 // …and snap-catches-up to here from the owner, inside the dead zone, on its approach side
export const ROAM_RADIUS_M = 2 // idle wanders stay within this disc of the owner (well inside the dead zone)
...
/** Fresh motion state for a newly-spawned pet: position seeds on the owner on the first finite frame. */
export const empty_pet_motion = () => ({ x: NaN, z: NaN, yaw: 0, roam_x: NaN, roam_z: NaN, roam_cd: 0 })

/**
 * A new idle-wander target uniformly inside the ROAM_RADIUS disc around the owner, plus the next interval.
...
/**
 * One pure steering step for a trailing pet. Given its current motion, the owner's position and dt, returns
 ...
 * @param {{ x: number, z: number }} owner the owner's world position (the follow target)
...
export function step_pet_follow(motion, owner, dt, rng = Math.random) {
  // First finite frame — spawn ON the owner (then it ambles out) instead of sliding in from NaN/origin.
