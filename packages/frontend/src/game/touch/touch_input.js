...
/** Module-level touch state. Consumers READ snapshots via the accessors below; exported mutators publish a
 *  replacement snapshot so an event callback never writes through state another callback may be reading. */
const create_touch_state = () => {
  let snapshot = {
...
const state = create_touch_state()
...
const publish_active = () => {
  const { armed, text_focused } = state.read()
  const now = armed && !text_focused
...
export const set_move = (forward, strafe) => {
  state.patch({ forward: clamp_axis(forward), strafe: clamp_axis(strafe) })
}
...
export const consume_mount_toggle = () => {
  const out = state.read().mount_toggle
  state.patch({ mount_toggle: false })
  return out
}
...
export const add_look = (dx, dy) => {
  const { look_dx, look_dy } = state.read()
  state.patch({
    look_dx: Number.isFinite(dx) ? look_dx + dx : look_dx,
    look_dy: Number.isFinite(dy) ? look_dy + dy : look_dy,
  })
}
...
export const read_movement = () => {
  const { forward, strafe, jump } = state.read()
