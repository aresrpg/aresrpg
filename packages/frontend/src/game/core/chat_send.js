...
// Outbound chat composition edge: reads the selected character, broadcasts through P2P, then echoes through
// the game reducer door. Kept outside modules/chat.js so the reducer pipeline never imports its own game root.
...
export function send_chat_message(message, channel = CHANNEL.general, target = '') {
  ...
  // No server resolves name/address for peers, so fill them from the chain roster and echo locally
  // (Trystero never delivers our own send back to us).
