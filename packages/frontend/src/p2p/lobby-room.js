...
//
// WHY OURS AND ONLY OURS: this layer shipped 2026-07-01 against five PUBLIC nostr relays, and the public
// relays rate-limited us into a fight stall on 07-27 — the transport was killed for a day and a client→server
// courier stood in, which is a client state write and therefore illegal (law 2). The convicted party was the
// third-party relay, never p2p: one self-hosted stateless broker turns that failure mode into infrastructure
// we run. There is exactly ONE relay URL and no fallback list — a dead relay must say DOWN, not degrade into
// a second system. The strategy is one import line (`@trystero-p2p/mqtt`); swapping brokers changes nothing else.
//
// FIGHTS NEVER RIDE THIS (standing law from the stall night). Fight TRUTH is chain→indexer→SSE; the only fight
// traffic here is the #49 courtesy PREVIEW of a peer's drafted turn, which the receiver sim-verifies and which
// no outcome depends on.
//
...
// The transport can silently die (a relay outage, or a WebRTC channel that freezes without a clean
...
function _build_room() {
  ...
  // Sample relay connectivity ONCE, a few seconds after join so the async WebSocket handshakes have settled.
  // Zero connected relays is not weather now that the relay is ours — it is an outage, and it says so.
  setTimeout(() => {
    ...
    if (connected.length === 0)
      game_log('p2p', `signaling relay UNREACHABLE at ${RELAY_URL} — no peer can be introduced`, { errors })
    else game_log('p2p', `joinRoom fired · relays connected: ${connected.length}/${relay_config.urls.length}`)
  }, 4000)
}
