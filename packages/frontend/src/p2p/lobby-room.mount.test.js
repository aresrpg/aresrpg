...
test('TR-97: a peer `state` declares exactly one cosmetic — the mount that earns its speed headroom', async () => {
  const { presence_character } = await import('../world-shell/presence_adapter.js')
  leave_lobby()
  reset_trystero_mock()
  join_lobby('0xMINE', { x: 0, y: 0 })
  fire_state({ id: '0xRIDER', mounted: true })
  expect(presence_character('0xRIDER')?.mounted).toBe(true)
  // A peer's word about APPEARANCE is not decoded at all: worn slots, pet and the veteran title resolve from
  // chain truth at the renderer, so a spoofed claim has nowhere to land.
  fire_state({ id: '0xLIAR', mounted: false, veteran: true, mount_glb: '/spoofed.glb' })
  const liar = presence_character('0xLIAR')
  expect(liar?.mounted).toBe(false)
  expect(liar?.veteran).toBeUndefined()
  expect(liar?.mount_glb).toBeUndefined()
  leave_lobby()
})
...
test('game traffic rides the data channel, never the signaling relay', async () => {
  leave_lobby()
  reset_trystero_mock()
  join_lobby('0xMINE', { x: 0, y: 0 })
