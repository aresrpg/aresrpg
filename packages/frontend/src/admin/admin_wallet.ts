...
export const reduce_admin_wallet = (admin: AdminState, input: AppInput): AdminState | null => {
  ...
  if (input.type === 'admin/wallet_connected' && admin.wallet.status === 'connecting')
    return Object.freeze({
      ...admin,
      wallet: Object.freeze({
        ...admin.wallet,
  ...
  if (input.type === 'admin/wallet_failed' && ['loading', 'connecting'].includes(admin.wallet.status))
    return Object.freeze({
      ...admin,
      wallet: Object.freeze({
        ...admin.wallet,
...
export const observe_admin_wallet = (
  ...
  events.on('STATE_UPDATED', (state, previous) => {
    ...
    if (wallet.status === 'connecting' && old_wallet.status === 'ready' && wallet.requested_wallet) {
      const request = ++generation
      const { requested_wallet } = wallet
      const selected = auth?.wallets().find(({ name }) => name === requested_wallet)
      if (!selected) return fail(new Error(`${wallet.requested_wallet} is unavailable`), request)
      void selected
        .connect()
        .then((session) => {
