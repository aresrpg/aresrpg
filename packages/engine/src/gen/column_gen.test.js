...
import { CHUNK_SIZE, CHUNKS_PER_COLUMN, SEA_LEVEL } from '../config/world_config.js'
import {
  VOXELS_PER_CHUNK,
...
import { create_chunk_store, build_neighbor_halos } from '../chunks/store.js'

import { region_islands, SKY_ISLANDS_CONFIG } from './sky_islands.js'
 ...
 * @param {import('./column_gen.js').GenContext} ctx
...
describe('sky-island fork: ground unchanged below y≈270, grass-crust over stone body (GEN_VERSION 5)', () => {
  ...
  function nearest_big_island(/** @type {import('./column_gen.js').GenContext} */ ctx) {
    const found = []
...
describe('column_gen output shape (frozen ChunkRecord contract §3.4)', () => {
  ...
  /**
   * @param {import('../chunks/format.js').ChunkRecord} ch
   * @param {0|1|2} axis
   * @param {number} row
   * @param {number} b
   * @returns {boolean}
...
describe('steep-alpine 6-axis face coverage (mesher emits a quad for every exposed solid face)', () => {
  ...
  test('with real neighbor halos, every solid face adjacent to non-solid is covered by exactly one quad — 0 missing, 0 dup', () => {
    ...
    for (let cy = 3; cy <= 7; cy += 1) {
      const chunk = /** @type {import('../chunks/format.js').ChunkRecord} */ (get(CX, cy, CZ))
