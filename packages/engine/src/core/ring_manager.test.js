...
import { MSG_GEN_REQUEST, MSG_MESH_REQUEST } from '../workers/rpc.js'
...
import { deserialize_mesh_job } from '../mesh/mesh_halo.js'

import { create_ring_manager, world_to_chunk_coord } from './ring_manager.js'
 ...
 * @param {(cx: number, cy: number, cz: number) => import('../chunks/format.js').ChunkRecord} record_for
 */
function create_fake_pool(record_for) {
  ...
  return {
    submitted,
    jobs,
    /** @type {import('../workers/pool.js').WorkerPool['submit']} */
    submit(type, payload) {
...
function create_fake_renderer() {
  ...
  return {
    ...
    upload_order,
    removed,
    /** @type {import('../render/pool_renderer.js').TerrainRenderer['upload_chunk']} */
    upload_chunk([cx, cy, cz]) {
      const key = coord_key(cx, cy, cz)
      uploaded.add(key)
      upload_order.push(key)
    },
    /** @type {import('../render/pool_renderer.js').TerrainRenderer['remove_chunk']} */
