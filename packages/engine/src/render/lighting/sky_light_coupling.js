...
import { sample_sky_rgb } from '../sky/sky_node.js'
 ...
 */
export function extinction_at(p, h_km) {
 ...
 */
export function sun_transmittance(mu, p = EARTH_ATMOSPHERE, cam_km = CAM_KM, steps = TRANSMITTANCE_STEPS) {
 ...
 */
export function moon_dir_of(sun_dir) {
 ...
 */
export function is_moon_key(sun_y) {
  return sun_y < 0
}
...
const NIGHT_SKYGLOW = [0.62, 0.78, 1.12]
...
let moon_mul = MOON_MUL_DEFAULT
let ambient_night_floor = AMBIENT_NIGHT_FLOOR_DEFAULT

/**
 * Live-retune the night moon-key + ambient-floor dials (taste pick). Null/omitted ⇒ keeps the current
 * values (the DEFAULT is the byte-identical baseline). Visual-only; couple_lighting reads these on its next call.
 * @param {{ moon_mul?: number, ambient_night_floor?: number }} [cfg]
 * @returns {void}
 */
...
export function current_night_lighting() {
  return { moon_mul, ambient_night_floor }
}
 ...
 */
export function shadow_intensity_for(y) {
 ...
 */
export function couple_lighting(sun_dir, base, params = EARTH_ATMOSPHERE) {
  const [, y] = sun_dir
  ...
  const sun_intensity = base.sun_intensity * (lum_mul + (moon_mul - lum_mul) * night)
  ...
  const sun_hue_norm = [T[0] / t_hue_max, T[1] / t_hue_max, T[2] / t_hue_max]
  const warm_tint = mix3([1, 1, 1], sun_hue_norm, warm_amount)
  const tint = mix3(warm_tint, NIGHT_SKYGLOW, night)
  const hemi_intensity = base.hemi_intensity * (amb_lum_mul + (ambient_night_floor - amb_lum_mul) * night)
  const hemi_sky = [base.hemi_sky[0] * tint[0], base.hemi_sky[1] * tint[1], base.hemi_sky[2] * tint[2]]
  const hemi_ground = [base.hemi_ground[0] * tint[0], base.hemi_ground[1] * tint[1], base.hemi_ground[2] * tint[2]]

  // ── WARM BACK-FILL — follows the sun's day/night level; colour kept (it is the shaded-side warmth) ──
  const fill_intensity = base.fill_intensity * (lum_mul + (moon_mul * 1.4 - lum_mul) * night)

  return {
    sun_color,
    sun_intensity,
    fill_color: base.fill_color.slice(),
    fill_intensity,
    hemi_sky,
    hemi_ground,
    hemi_intensity,
