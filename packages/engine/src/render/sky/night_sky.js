...
import { hash01 } from '../texture_noise.js'

import { SKY_NIGHT, MOON_DISC_COS, MOON_DISC_INNER_COS, MOON_DISC_RGB, disc_space_uv } from './sky_node.js'
...
export const MOON_SKY_GLOW_RGB = [0.58, 0.7, 1.0]
...
export const RING_RGB = [0.9, 0.78, 0.55]
 ...
 * clustering carry "more stars"); worst in-band star+band+nebula+base = 1.92 < the 2.05 bloom knee
 * (unit-tested against THIS config). NIGHT_SKY_NEUTRAL above is retained as the pre-pick reference.
 * @type {NightSkyCfg}
 */
export const NIGHT_SKY_LIVE = {
  base_palette: { zenith: [0.004, 0.005, 0.017], horizon: [0.011, 0.011, 0.03], nadir: [0.004, 0.005, 0.015] },
  star_thresh_shift: -0.06,
  star_bright_mul: 1.0,
  star_cluster: 0.35,
  star_red_giant: 0.06,
  // in-engine tune (2026-07-13 ship gate): the hillaire physical-sky night base + HIGH-tier bloom WASH the
  // band to a pale grey column, drowning the colour. Taming mw (0.18→0.10) kills the grey bloom, and pushing
...
export function night_gate_js(sun_y) {
...
export function moon_sky_glow_js(cos_vm) {
...
export function moon_halo_js(cos_vm) {
...
export function star_moon_fade_js(cos_vm, moon_up) {
...
export function horizon_fade_js(view_y, hf) {
  if (!hf) return 1
 ...
 */
export function derive_night_sky_params(seed) {
  const codes = typeof seed === 'number' ? [seed | 0] : [...String(seed)].map((c) => c.codePointAt(0) ?? 0)
  const h = (/** @type {number} */ i) => hash01(...codes, i)
  ...
  const galaxy_n = [gr * Math.cos(gaz), gy, gr * Math.sin(gaz)]
  ...
  const planets = [0, 1].map((i) => {
    ...
    const up = Math.abs(n_i[1]) > 0.9 ? [1, 0, 0] : [0, 1, 0]
 ...
 */
export function planet_dir_js(p, sun_az) {
...
export function create_night_sky_node({
  ...
  const c3 = (/** @type {number[]} */ c) => vec3(c[0], c[1], c[2])
  ...
  for (const tier of STAR_TIERS) {
    ...
    /** @type {*} */ let contrib = core.mul(core).mul(exists).mul(bright).mul(twinkle).mul(band_bright)
  ...
  if (q && cfg.nebula.intensity > 0) {
    ...
    // orange galactic core: a warm bloom around galaxy_a (a fixed point ON the band), exp falloff.
    const core = exp(
      float(1)
  ...
  let sky_add = base.add(sky_glow).add(halo).add(stars).add(milky_way)
