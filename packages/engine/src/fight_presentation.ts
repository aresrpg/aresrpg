...
export const create_fight_presentation = ({
  entities,
  vfx,
}: Readonly<{ entities: EntityLayer; vfx: TransientEffects }>) =>
  Object.freeze({
    play: async (cue: FightPresentationCue): Promise<boolean> => {
      ...
      if (cue.type === 'movement') {
        if (cue.mode === 'teleport' || cue.mode === 'swap' || cue.mode === 'place')
          return entities.snap(cue.entity_id, cue.cells.at(-1))
      ...
      if (cue.type === 'zone') return vfx.play_zone(cue)
      return true
    },
  })
