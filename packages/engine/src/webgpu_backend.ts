...
export const create_webgpu_backend = async (
    ...
    cleanup
      ...
      .forEach((release) => {
        try {
          ...
          console.error('WebGPU resource cleanup failed.', error)
        }
      })
  }
  ...
  try {
    ...
    // Three has not requested a canvas context yet. Its dispose() would re-enter failed init.
    disposed = true
  ...
  try {
    ...
    const character_crowd = own(create_character_crowd_layer({ scene }))
    ...
    const portal = presentation === 'world' ? own(create_portal({ scene, world: compiled_world })) : null
    const dungeon_portals = own(create_dungeon_portals({ scene, world: compiled_world }))
    ...
    const cancel_chunk = (key: string): void => {
    ...
    const schedule_mesh = (chunk: RenderChunkRequest, revision: number): void => {
      ...
      void mesh_pool
        ...
        .catch((error: unknown) => {
          ...
          console.error(`Failed to build chunk ${chunk.key}.`, error)
        })
    }
