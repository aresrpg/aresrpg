...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)('IMPACT_FRAMES table', () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)('resolve_impact — mapped anims resolve at impact, not end', () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)('resolve_impact — LOUDNESS on an UNKNOWN anim (W4 keystone)', () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'loudness is keyed off resolve_impact.known === false (unknown anim only — never a clipless mob)',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)('hit never resolves to an attack-family clip (D304 superseded)', () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)('D303 — resolve_gait (clip-aware timeScale, no foot-slide)', () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)('make_outline_material — inverted-hull silhouette (flat black rim)', () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'reaction_for — damage flinches + flashes, death flashes only, heal is a green pulse',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'recoil_away_dir — the struck body jerks AWAY from the attacker it faces',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'recoil_envelope — a fast jerk out, an eased spring back (0 at both ends)',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)('darken_to_luminance — dark team rim (darker outlines)', () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'float_text_magnitude — parses a fully-composed float string for SIZING only',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'float_magnitude_scale — small ticks read small, big hits read big ("3-damage vs 40-crit")',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'float_pop_curve — spring OVERSHOOT pop-in (0 → peak → settle), the "bouncy" fix',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)('float_rise_ease — ease-out arc (fast rise, settles near the top)', () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'float_opacity_curve — HANG at full opacity, then FADE-DROP (accelerating)',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'float_gravity_drop_curve — the small ballistic sag during the fade tail',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'float_burst_stagger — multi-hit numbers fan out ~80ms apart, not a clump',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'recoil_jitter — a decaying SHAKE oscillation, never a residual offset ("doesn\'t shake")',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'reaction_for — crit now gets its OWN gold tint (was lumped in with plain damage)',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'flash_envelope — rises then decays to EXACTLY 0 (the "material state restored" contract)',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'should_procedural_death — ladder rung 2 for a DEATH beat (mirrors the hit ladder)',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'should_procedural_attack — ladder rung 2 for an ATTACK beat (a clipless attacker never strikes as a statue)',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'peak_envelope — the shared out-and-back curve (recoil + clipless lunge)',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'arm_attack_lunge / advance_lunge — clipless-attacker strike (fires toward facing, restores exactly)',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'should_force_remove — the hard removal belt ("a dead mob may NEVER persist")',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'react_to_impact + advance_recoil — the procedural flinch FIRES for a clipless rig and restores exactly',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'advance_flash — material state restores its exact baseline after the tint',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'arm_death_response + advance_death_collapse — procedural collapse for a clipless death only',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'render_position_of — the entity-anchor position feed (LIVE render XZ, never the logical cell)',
  () => {
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)(
  'worn cosmetics — the player fight rig mounts equipped hat/cloak (the roam parity that was missing)',
  () => {
    const make_board = () => ({
      cell_center_world: (/** @type {number} */ x, /** @type {number} */ y) => [x * 1.33 + 0.665, 0, y * 1.33 + 0.665],
      origin: { x: 0, y: 0, z: 0 },
