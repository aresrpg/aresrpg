...
describe('select_build_frontier — coarse-first coverage (near-mid empty-band regression)', () => {
  ...
  const far = 2048
  const key = (/** @type {import('../../src/lod/quadtree.js').Selection} */ s) => `${s.level},${s.sx},${s.sz}`
  /** Does section `s`'s footprint contain the world point (px,pz)? */
  const covers = (
    /** @type {import('../../src/lod/quadtree.js').Selection} */ s,
    /** @type {number} */ px,
    /** @type {number} */ pz
  ) => {
...
/** @typedef {import('../../src/lod/quadtree.js').Selection} Selection */
