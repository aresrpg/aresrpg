...
import { create_shoulder_camera, CAMERA_RIG_CONSTANTS } from '../../src/player/camera_rig.js'
 ...
 *  @param {import('../../src/player/camera_rig.js').ShoulderCamera} cam */
function settle(cam) {
  let pose
  for (let i = 0; i < 4; i += 1) {
    pose = cam.update({ feet: FEET, eye_height: EYE_H, speed: 0, solid_at: NO_WALL, dt: 1 / 60 })
  }
  return /** @type {import('../../src/player/camera_rig.js').CameraPose} */ (pose)
}
...
/** Fully converge the follow spring at the constant pivot. @param {import('../../src/player/camera_rig.js').ShoulderCamera} cam */
 ...
 *  (smaller halflife → faster catch → larger step). @param {import('../../src/player/camera_rig.js').ShoulderCamera} cam */
...
describe('TR-1 cinematic camera (trailer mode)', () => {
  ...
  it('deeper zoom-OUT: cinematic pulls past the normal MAX_DIST, up to the cinematic ceiling', () => {
    ...
    let pose = /** @type {import('../../src/player/camera_rig.js').CameraPose} */ (/** @type {unknown} */ (FRAME()))
...
describe('S-76b camera wall margin (near-plane x-ray)', () => {
  it('3P squeeze: the arm stops with the eye ≥ margin off a wall plane (never inside it)', () => {
    ...
    const p = /** @type {import('../../src/player/camera_rig.js').CameraPose} */ (pose)
  ...
  it('3P corner sweep: a lone block never gets a face inside the margin, ANY approach azimuth', () => {
    ...
    for (let k = 0; k < 24; k += 1) {
      ...
      const [ex, ey, ez] = /** @type {import('../../src/player/camera_rig.js').CameraPose} */ (pose).position
 ...
 *  both the settled pivot and the fingerprint step). @param {import('../../src/player/camera_rig.js').ShoulderCamera} cam
 *  @param {number} speed */
