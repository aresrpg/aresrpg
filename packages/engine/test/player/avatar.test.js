...
const { create_character_controller } = /** @type {typeof import('../../src/player/character_controller.js')} */ (
  SENSHI_MALE_GLB_AVAILABLE ? await import('../../src/player/character_controller.js') : {}
)
...
describe.skipIf(!SENSHI_MALE_GLB_AVAILABLE)('character avatar swim clip selection', () => {
  ...
  test('falls back to WALK while moving underwater when no SWIM clip exists', async () => {
    ...
    avatar.update(
      /** @type {import('../../src/player/character_avatar.js').AvatarAnim} */ (transform.anim),
      transform.facing_yaw,
      0.3
    )
