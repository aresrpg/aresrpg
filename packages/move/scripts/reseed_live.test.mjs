...
import { build_transaction, fetch_world_state } from './reseed_live.mjs'
...
const targets = (tx) =>
  tx
    ...
    .commands.map((command) =>
      command.$kind === 'MoveCall' ? `${command.MoveCall.module}::${command.MoveCall.function}` : command.$kind
    )
...
describe('reseed live composer — every planner-emittable world call composes', () => {
  ...
  test('an empty boss mask still composes — clearing the mask is a meaningful state', () => {
    const tx = build_transaction({ calls: [world_call('set_boss_mask', { rows: [] })] }, context)
    expect(targets(tx)).toEqual(['world::set_boss_mask'])
  })

  test('a call the planner cannot emit still throws BEFORE any signing', () => {
    expect(() => build_transaction({ calls: [world_call('drop_the_world', {})] }, context)).toThrow(
      /unknown world authoring call/
    )
  })
})
...
const child_id = derive_dynamic_field_id(VERSIONED_ID, 'u64', bcs.u64().serialize(WORLD_VERSION).toBytes())
  ...
  async getObjects({ objectIds }) {
    return {
      objects: objectIds.map((id) => {
        ...
        if (id === child_id) return { json: { value: { mobs: [{ template_id: TEMPLATE }], mob_levels: [42] } } }
        return { json: null }
      }),
    }
  },
...
describe('reseed convergence — the fetched payload feeds the planner with zero drift', () => {
  ...
  test('a world already matching the seed plans NOTHING (the rerun the ceremony requires)', async () => {
    ...
    const leg = build_world_leg({
      seed_rows: [{ id: 'w', resources: [], mobGroups: [{ mob: 'm', rate: 0.01 }], dungeonRooms: [] }],
