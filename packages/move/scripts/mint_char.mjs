...
// Ids come from the ceremony manifest/env: ARESRPG_PACKAGE_ID / ARES_ROOT / CHARACTER_POLICY / VERSION
// (+ optional SENDER).
...
const sui = getClient('testnet')
// KioskClient cannot take the gRPC client; use the same GraphQL-compatible construction client as the SDK.
// The transaction build, simulation, execution, and finality read all stay on `sui` (gRPC).
...
if (process.env.DRY_RUN === '1') {
  ...
  const dr = normalizeReceipt(
    await sui.simulateTransaction({
      transaction: tx,
  ...
  console.log(
    ...
    (dr.objectChanges || [])
      ...
      .map((c) => c.objectType?.split('::').slice(-1)[0])
      .join(', ')
  )
} else {
