...
async function fetch_json(url, fetch_impl) {
  ...
  if (!response.ok) throw new Error(`${url}: HTTP ${response.status}; no chain writes attempted`)
...
function normalize_sale(row) {
  ...
  const supply_remaining = row?.supply_remaining == null ? null : Number(row.supply_remaining)
  ...
  if (!sale_id || !template_id) throw new Error('shop API row lacks sale_id/template_id')
  if (!Number.isSafeInteger(minted) || minted < 0) throw new Error(`sale ${sale_id} has invalid minted`)
  if (supply_remaining != null && (!Number.isSafeInteger(supply_remaining) || supply_remaining < 0))
    throw new Error(`sale ${sale_id} has invalid supply_remaining`)
  if (typeof paused !== 'boolean') throw new Error(`sale ${sale_id} has invalid paused`)
  return {
    sale_id,
    template_id,
    price_mist,
...
export async function fetch_live_rows(fetch_impl = fetch) {
  ...
  if (!Array.isArray(shop?.sales)) throw new Error('/v1/shop response has no sales array')
  if (!Array.isArray(encyclopedia?.items)) throw new Error('/v1/encyclopedia response has no items array')
  ...
  for (const raw_template of encyclopedia.items) {
    ...
    if (template_by_id.has(template.template_id))
      throw new Error(`/v1/encyclopedia returned duplicate template ${template.template_id}`)
