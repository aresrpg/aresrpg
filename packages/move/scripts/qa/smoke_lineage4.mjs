...
const ctx = { network, kiosk_client: new KioskClient({ client: sui_client, network, packageIds: { personalKioskRulePackageId: M._rules } }) }
const created = (r, needle, ownedOnly = false) =>
  (r.objectChanges || []).find(
    (c) => c.type === 'created' && (c.objectType || '').includes(needle) && (!ownedOnly || c.owner?.AddressOwner === ADDR),
  )?.objectId
...
async function main() {
  ...
  await run(sui_client, keypair, 'admin:set_price', (() => {
    const tx = new Transaction()
    tx.moveCall({ target: `${L}::creation::set_price`, arguments: [tx.object(CAP), tx.object(CREATION), tx.pure.u64(50_000_000n), tx.object(VER)] })
    return tx
  })(), { ceilingSui: 0.2 })
  ...
  let zx, zy, nodes = 9, groups = 1
  const mk_search = () => { const t = search_zone_ptb(ctx)({ world_id: WORLD, kiosk_id, personal_kiosk_cap_id, character_id, x, z }); t.setSender(ADDR); t.setGasBudget(300_000_000n); return t }
  const sdry = await sui_client.dryRunTransactionBlock({ transactionBlock: await mk_search().build({ client: sui_client }) })
  if (sdry.effects?.status?.status === 'success') {
    const sr = await sui_client.signAndExecuteTransaction({ signer: keypair, transaction: mk_search(), options: { showEffects: true, showEvents: true } })
    await sui_client.waitForTransaction({ digest: sr.digest })
    zx = Number(evt(sr, '::zones::ZoneSearched', 'zx')); zy = Number(evt(sr, '::zones::ZoneSearched', 'zy'))
    nodes = Number(evt(sr, '::zones::ZoneSearched', 'resource_nodes')); groups = Number(evt(sr, '::zones::ZoneSearched', 'mob_groups'))
    ...
    const zof = new Transaction(); zof.moveCall({ target: `${L}::world::zone_of`, arguments: [zof.object(WORLD), zof.pure.u32(x), zof.pure.u32(z)] })
    const zi = await sui_client.devInspectTransactionBlock({ sender: ADDR, transactionBlock: zof })
    zx = Buffer.from(zi.results[0].returnValues[0][0]).readUInt32LE(); zy = Buffer.from(zi.results[0].returnValues[1][0]).readUInt32LE()
    console.log(`  search skipped (${sdry.effects?.status?.error}) — zone (zx=${zx}, zy=${zy}) already discovered; probing its existing nodes/groups`)
  }
  ...
  if (groups > 0) {
    const FIGHT_REG = M.engine.shared.FightRegistry, EVER = M.engine.version, BRUTE = G.tool_brute
    const insp_tx = new Transaction()
    insp_tx.moveCall({ target: `${L}::zones_view::mob_spawn_id`, arguments: [insp_tx.object(WORLD), insp_tx.pure.u32(zx), insp_tx.pure.u32(zy), insp_tx.pure.u64(0n)] })
    ...
    const ticket = f_tx.moveCall({ target: `${L}::zones::claim_mob_group`, arguments: [f_tx.object(WORLD), f_tx.object(kiosk_id), f_tx.object(personal_kiosk_cap_id), f_tx.pure.id(character_id), f_tx.pure.u64(spawn_id), f_tx.object(CFG), f_tx.object(VER), f_tx.object.clock()] })
    f_tx.moveCall({ target: `${L}::fight::create`, arguments: [f_tx.object(FIGHT_REG), ticket, f_tx.object(WORLD), f_tx.object(kiosk_id), f_tx.object(personal_kiosk_cap_id), f_tx.object(BRUTE), f_tx.pure.bool(true), f_tx.moveCall({ target: '0x1::option::none', typeArguments: ['0x2::object::ID'] }), f_tx.makeMoveVec({ type: '0x2::object::ID', elements: [] }), f_tx.object(CFG), f_tx.object(VER), f_tx.object(EVER), f_tx.object.clock()] })
    f_tx.setSender(ADDR); f_tx.setGasBudget(300_000_000n)
    ...
    console.log(`  fight create dry-run (brute ${BRUTE.slice(0, 10)}…): ${fst?.status}${fst?.error ? ' — ' + fst.error : ''}`)
  }
  ...
  for (let node_index = 0; node_index < Math.max(1, nodes); node_index++) {
    const g_tx = gather_ptb(ctx)({ world_id: WORLD, kiosk_id, personal_kiosk_cap_id, character_id, zx, zy, node_index, template_id: IRON, rare_template_id: GOLDEN })
...
main().catch((e) => { console.error(`\nSMOKE STOPPED: ${e.message}`); process.exit(1) })
