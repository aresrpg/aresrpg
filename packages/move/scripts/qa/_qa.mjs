...
export async function exec(tx, label = '') {
  ...
  const gasMist = gas ? Number(gas.computationCost) + Number(gas.storageCost) - Number(gas.storageRebate) : 0
  logline(
    `${status === 'success' ? 'OK ' : 'FAIL'} ${label} digest=${r.digest} gasMist=${gasMist}${err ? ' err=' + err : ''}`
  )
  return {
    ok: status === 'success',
    status,
