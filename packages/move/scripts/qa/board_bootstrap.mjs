...
const L = M.forgemagie.latest ?? M.forgemagie.pkg, CORE = M.aresrpg.latest ?? M.aresrpg.pkg, CAP = M.aresrpg.admin, VER = M.aresrpg.version
...
if (!board) {
  ...
  tx.moveCall({ target: `${CORE}::config::set_forge_brand`, typeArguments: [`${L}::forgemagie::Forge`], arguments: [tx.object(CAP), tx.object(M.aresrpg.shared.GameConfig), tx.object(VER)] })
  ...
  const created = (r.objectChanges || []).find(c => c.type === 'created' && c.objectType.endsWith('::forgemagie::CrushBoard'))
  ...
  S.crushBoard = board; S.crushBoardVersion = board_version
...
tx2.moveCall({ target: `${L}::forgemagie::register_rune`, arguments: [tx2.object(CAP), tx2.object(board), tx2.pure.id(S.items.rune_might), tx2.pure.u8(2), tx2.pure.u8(1), tx2.object(VER)] })
tx2.moveCall({ target: `${L}::forgemagie::register_rune`, arguments: [tx2.object(CAP), tx2.object(board), tx2.pure.id(S.items.rune_guard), tx2.pure.u8(13), tx2.pure.u8(1), tx2.object(VER)] })
await run(sui_client, keypair, 'forgemagie:register_runes', tx2, { ceilingSui: 0.05 })
S.crushBoard = board; fs.writeFileSync(new URL('../out/seed_manifest.json', import.meta.url), JSON.stringify(S, null, 2))
