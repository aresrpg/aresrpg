...
fun run_batch_claim(box_count: u32, count: u32, invalid: u8) {
  ...
  let box_id = object::id(&inventory);
  item::deposit(&mut kiosk, cap, &policy, option::none(), inventory);
  api::open_loot_boxes(&tables, &mut kiosk, &personal, box_id, &boxes, count, &protected, &randomness, &version, scenario.ctx());
  assert!(sui::event::events_by_type<loot_box::LootBoxOpened>().length() == (count as u64), 20);
  let batch = sui::event::events_by_type<loot_box::LootBoxesOpened>();
  assert!(batch.length() == 1, 21);
  if (count == 2) std::debug::print(&std::bcs::to_bytes(&batch[0]));
  if (box_count == count) assert!(!kiosk.has_item(box_id), 22)
  else assert!(item::amount(kiosk.borrow(cap, box_id)) == box_count - count, 23);
  test_scenario::return_shared(tables);
  test_scenario::return_shared(version);
  test_scenario::return_shared(randomness);
  transfer::public_transfer(kiosk, OWNER);
...
#[test, expected_failure(abort_code = 2910, location = aresrpg::loot_box)]
fun batch_over_limit_refused() { run_batch(51, 51); }
#[test, expected_failure(abort_code = 203, location = aresrpg::item)]
fun batch_insufficient_stack_refused() { run_batch(2, 3); }
