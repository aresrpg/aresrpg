...
const ERooted: u64 = 2603; // gathering/ambush blocks consumption; dungeon confinement blocks teleportation
...
fun burn_one(
  ...
  assert_available(kiosk.borrow(cap, character_id), &effect, clock);
  let category = item::burn(kiosk, cap, protected_item, item_id, 1, ctx);
  assert!(category == b"consumable".to_string(), ENotConsumable);
  effect
}

fun assert_available(character: &Character, effect: &consumable_effect::Effect, clock: &Clock) {
  // Kiosk custody proves the character is between fights. The dungeon root confines travel,
  // while gathering and ambush roots still refuse every ordinary consumable before burning.
  let available = if (dungeon::has_run(character)) {
    consumable_effect::heal_amount(effect).is_some()
      || consumable_effect::is_reset_stats(effect)
      || consumable_effect::is_reset_spells(effect)
  } else !world::is_rooted(character, clock);
  assert!(available, ERooted);
}

#[test_only]
public(package) fun assert_available_for_testing(character: &Character, clock: &Clock) {
  assert_available(character, &consumable_effect::heal(1), clock);
}
