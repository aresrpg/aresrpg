...
fun mob_permanent_and_timed_alters_and_expiry() {
  ...
  // #2000 — the authored 2 covers the mob's next TWO turns; #2033 — the second of them ENDS the coverage and
  // re-derives back to the PERMANENT base 30.
  whole_turn(&mut fight, true, 0);
  assert!(mob_earth_res(&fight, 0) == 20, 2); // one covered turn still to come
  whole_turn(&mut fight, true, 0);
  assert!(mob_earth_res(&fight, 0) == 30, 3); // collected → permanent base (not the debuffed 20, not the pristine 50)
...
fun timed_strength_shred_softens_mob_outgoing_then_expires() {
  ...
  // expire the shred (#2000: an authored 2 covers two further turns; #2033: collected when the 2nd ends) → 100.
  whole_turn(&mut fight, true, 0);
  whole_turn(&mut fight, true, 0);
...
fun ap_drain_persists_through_begin_turn_and_clamps() {
  ...
  // that turn's END collects the spent row (#2033), so the FOLLOWING turn's refill is full.
...
fun ally_feed_survives_begin_turn() {
  ...
  // that turn's END collects the spent credit row (#2033), so the NEXT refill is back to base.
