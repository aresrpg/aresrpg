...
public fun create<W: drop>(
  ...
  create_inner(_w, registry, world, spawn_id, world_seed, anchor_x, anchor_z, spawned_at_ms, is_public, party_id, gated_joins, spec, group_size, group_seed, content_template, creator, creator_lines, creator_category, dials, version, clock, ctx);
}
...
// §387 — the seat's equipped-weapon FINE CATEGORY, the key `cast::weapon_strike` derives the shape cell set from.
// A per-seat DYNAMIC FIELD, upgrade-safe alongside the frozen `Participant`/`Weapon` layout exactly like the
// weapon lines above (the family already drives reach/element/AP via `Weapon`; only the SHAPE needs the raw slug).
// Absent (a tool / bare hands / a pre-§387 seat) ⇒ `none` ⇒ the 1-cell default, so an un-authored seat strikes
