...
fun honest_walk_accepted_teleport_rejected() {
  ...
  let cp = world::y70(0, 0, 1000, false); // at origin, t=1000ms, no pet
...
fun pet_mount_needs_both_ends() {
  let mut sc = ts::begin(OWNER);
  let w = world_at_speed_1000(&mut sc);
  let cp = world::y70(0, 0, 1000, true); // pet WAS equipped at the checkpoint
...
fun verify_travel_too_far_aborts() {
  let mut sc = ts::begin(OWNER);
  let w = world_at_speed_1000(&mut sc);
  let cp = world::y70(0, 0, 1000, false);
