...
/// travel-verified from the checkpoint and the checkpoint ADVANCES there — the S-71 walked-to-zone z502; a
...
fun search_non_gather_entry_single_cell_one_harvest() {
  ...
  zones::z52(&mut w, zx, zy, 0); // harvest it once
...
fun consume_resource_cell_twice_aborts() {
  ...
  let mut w = sc.take_shared<World>();
  zones::z52(&mut w, zx, zy, 0);
  zones::z52(&mut w, zx, zy, 0); // ENodeEmpty
  abort
}
