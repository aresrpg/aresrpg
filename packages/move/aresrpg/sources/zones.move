...
public(package) fun z48(t: GroupTicket): (ID, ID, u64, ID, u32, u32, u16, u64, u64) {
...
// name shortened 2026-07-27: aresrpg at Sui object-size ceiling (republish restructure); see the growth row
/// Unpack a `MemberGroupTicket` → the `z48` tuple plus `(members, progress)`. Same seam law:
/// the security is the CREATION gate, so unpacking outside `fight::open_group` only forfeits the claim.
public(package) fun z49(t: MemberGroupTicket): (ID, ID, u64, ID, vector<ID>, u64, u32, u32, u16, u64, u64) {
...
entry fun join_world(
  ...
  z95(world, kiosk, pkcap, character_id, config, version, clock, &mut gen);
}
...
fun z95(
  ...
  let (x, z) = if (first_join) {
    ...
    character_link::z2(character, wid, world::z44(sx, sz, now, pet), version);
    (sx, sz)
  } else {
  ...
  character_link::z1(character, wid, version);
...
entry fun search_zone(
  ...
  let mut gen = random::new_generator(r, ctx);
  z96(world, kiosk, pkcap, character_id, x, z, config, version, clock, &mut gen);
}
...
fun z96(
  ...
  // ── write: advance the checkpoint to the PROVEN standing position (§5 — discovery writes a checkpoint) ──
  {
    let character = kiosk.borrow_mut(owner_cap, character_id);
    let pet = equipment::pet_equipped(character);
    character_link::z2(character, wid, world::z44(x, z, now, pet), version);
  };
  ...
  let (msids, mt, mmembers, mx, mz, ms, mg, _progress) =
    zone_comp::z46(world, zx, zy, seed, config.team_size_bound());
  let (rsids, _rt, _rx, _rz, _rj, _rr) = zone_comp::z47(world, zx, zy, seed);
  ...
  z105(wuid, zx, zy); // the re-roll renames every spawn — the old rounds name nothing
...
public fun claim_mob_group(
  ...
  z100(world, kiosk, pkcap, character_id, option::none(), spawn_id, option::none(), config, version, clock)
}
...
public fun claim_mob_group_in_zone(
  ...
  z100(world, kiosk, pkcap, character_id, option::some(ZoneKey { zx, zy }), spawn_id, option::none(), config, version, clock)
}
...
public fun claim_mob_group_with_proof(
  ...
  z100(world, kiosk, pkcap, character_id, option::none(), spawn_id, option::some(p), config, version, clock)
}
...
public fun claim_mob_group_in_zone_with_proof(
  ...
  let p = GroupClaimProof { index, template, x, z, group_size, group_seed, proof };
  z100(world, kiosk, pkcap, character_id, option::some(ZoneKey { zx, zy }), spawn_id, option::some(p), config, version, clock)
}
...
public fun claim_mob_group_members(
  ...
  z97(world, kiosk, pkcap, character_id, option::none(), spawn_id, config, version, clock)
}
...
public fun claim_mob_group_in_zone_members(
  ...
  z97(world, kiosk, pkcap, character_id, option::some(ZoneKey { zx, zy }), spawn_id, config, version, clock)
}

// name shortened 2026-07-27: aresrpg at Sui object-size ceiling (republish restructure); see the growth row
/// The member security tail — `z100`'s twin, differing only in what it authenticates (a roster
...
fun z97(
  ...
  let (wid, template, members, progress, x, z, group_size, spawned_at_ms, group_seed) =
    z101(world, kiosk, pkcap, character_id, zone, spawn_id, option::none(), true, config, version, clock);
  MemberGroupTicket {
...
fun z98(world: &World, zx: u32, zy: u32, spawn_id: u64, team_bound: u64, members: bool): (ID, vector<ID>, u64, u32, u32, u16, u64, u64, u64) {
  let key = ZoneKey { zx, zy };
  assert!(df::exists(world::uid(world), key), ESpawnNotFound);
  let zone: &Zone = df::borrow(world::uid(world), key);
  let (sids, tpls, rosters, xs, zs, sizes, gseeds, progress) = if (members) {
    zone_comp::z46(world, zx, zy, zone.seed, team_bound)
  } else {
  ...
  while (i < n) {
    if (sids[i] == spawn_id) {
      assert!(!z530(&zone.mob_bitmap, i), ESpawnNotFound); // consumed = gone (no double-fight of one group)
...
// name shortened 2026-07-27: aresrpg at Sui object-size ceiling (republish restructure); see the growth row
/// `z98` projected to the single-spec shape the original (format-1/2) ticket carries.
fun z99(world: &World, zx: u32, zy: u32, spawn_id: u64, team_bound: u64): (ID, u32, u32, u16, u64, u64, u64) {
  let (t, _roster, _progress, x, z, group_size, spawned_at_ms, group_seed, index) =
    z98(world, zx, zy, spawn_id, team_bound, false);
...
fun z100(
  ...
  let (wid, template, _members, _progress, x, z, group_size, spawned_at_ms, group_seed) =
    z101(world, kiosk, pkcap, character_id, zone, spawn_id, claim_proof, false, config, version, clock);
...
fun z101(
  ...
  let (template_id, roster, progress, mx, mz, group_size, spawned_at_ms, group_seed, index) = if (members) {
    z98(world, zx, zy, spawn_id, config.team_size_bound(), true)
  } else {
    let (t, x, z, gs, sa, g, ix) = z102(world, zx, zy, spawn_id, claim_proof, config.team_size_bound());
  ...
  z103(world, zx, zy, index);
  {
    let character = kiosk.borrow_mut(owner_cap, character_id);
    let pet = equipment::pet_equipped(character);
    character_link::z2(character, wid, world::z44(mx, mz, now, pet), version);
  };
...
fun z102(
  ...
  if (claim_proof.is_none()) {
    ...
    return z99(world, zx, zy, spawn_id, team_bound)
  };
  ...
  assert!(!z530(&zone.mob_bitmap, index), ESpawnNotFound);
...
fun z103(world: &mut World, zx: u32, zy: u32, index: u64) {
  let zone: &mut Zone = df::borrow_mut(world::uid_mut(world), ZoneKey { zx, zy });
  z531(&mut zone.mob_bitmap, index);
}
...
public(package) fun z50(world: &mut World, zx: u32, zy: u32, index: u64, spawn_id: u64, x: u32, z: u32) {
  assert!(!mob_group_live(world, zx, zy, index), EGroupNotConsumed);
  let wid = object::id(world);
  let wuid = world::uid_mut(world);
  {
    let zone: &mut Zone = df::borrow_mut(wuid, ZoneKey { zx, zy });
    z532(&mut zone.mob_bitmap, index);
  };
  let round = z104(wuid, zx, zy, spawn_id);
...
fun z104(wuid: &mut UID, zx: u32, zy: u32, spawn_id: u64): u64 {
  let key = ZoneRoundsKey { zx, zy };
...
fun z105(wuid: &mut UID, zx: u32, zy: u32) {
  let key = ZoneRoundsKey { zx, zy };
  if (df::exists(wuid, key)) {
...
public(package) fun z51(world: &World, zx: u32, zy: u32, node_index: u64): (u32, u32, u8, u8, ID) {
  let key = ZoneKey { zx, zy };
  ...
  assert!(!z530(&zone.res_bitmap, node_index), ENodeEmpty);
...
/// harvest. The single live caller (`gathering::gather`) bounds `node_index` via `z51` in the SAME
...
public(package) fun z52(world: &mut World, zx: u32, zy: u32, node_index: u64) {
  let key = ZoneKey { zx, zy };
  let wuid = world::uid_mut(world);
  assert!(df::exists(wuid, key), EBadNode);
  let zone: &mut Zone = df::borrow_mut(wuid, key);
  assert!(!z530(&zone.res_bitmap, node_index), ENodeEmpty);
  z531(&mut zone.res_bitmap, node_index);
}
...
public fun zone_discovered_at(world: &World, zx: u32, zy: u32): u64 { z107(world, zx, zy).discovered_at_ms }
public fun zone_seed(world: &World, zx: u32, zy: u32): u64 { z107(world, zx, zy).seed }
...
public fun mob_bitmap_bytes(world: &World, zx: u32, zy: u32): u64 { z107(world, zx, zy).mob_bitmap.length() }
public fun res_bitmap_bytes(world: &World, zx: u32, zy: u32): u64 { z107(world, zx, zy).res_bitmap.length() }
...
public fun mob_group_live(world: &World, zx: u32, zy: u32, i: u64): bool {
  !z530(&z107(world, zx, zy).mob_bitmap, i)
}
...
public fun resource_remaining(world: &World, zx: u32, zy: u32, i: u64): u16 {
  if (z530(&z107(world, zx, zy).res_bitmap, i)) 0 else 1
}
...
public(package) fun derive_mobs(world: &World, zx: u32, zy: u32, seed: u64, team_bound: u64): (vector<u64>, vector<ID>, vector<u32>, vector<u32>, vector<u16>, vector<u64>) {
  ...
  if (format == 3) { // 3 = member lists — project them away; ids, positions and sizes are what this door promises
    let (sids, tpls, _members, xs, zs, sizes, gseeds, _progress) = zone_comp::z46(world, zx, zy, seed, team_bound);
    (sids, tpls, xs, zs, sizes, gseeds)
  } else if (format == 2) { // 2 = zone_gen lattice commitment
    zone_comp::z45(world, zx, zy, seed, team_bound)
  } else {
...
public(package) fun derive_res(world: &World, zx: u32, zy: u32, seed: u64): (vector<u64>, vector<ID>, vector<u32>, vector<u32>, vector<u8>, vector<u8>) {
  ...
  if (format == 2 || format == 3) {
    zone_comp::z47(world, zx, zy, seed)
  } else {
...
fun z106(world: &World, zx: u32, zy: u32): u8 {
...
// name shortened 2026-07-27: aresrpg at Sui object-size ceiling (republish restructure); see the growth row
fun z107(world: &World, zx: u32, zy: u32): &Zone {
...
fun z530(bm: &vector<u8>, i: u64): bool {
  let byte = i / 8;
...
fun z531(bm: &mut vector<u8>, i: u64) {
  let byte = i / 8;
...
// name shortened 2026-07-27: aresrpg at Sui object-size ceiling (republish restructure); see the growth row
/// Clear bit `i`, then pop trailing zero bytes — the exact inverse of `z531`, so a released group leaves the
/// bitmap byte-identical to what it was before its claim (the cost shape stays lazy, and the JS mirror's
/// `z530` reads a shorter vector as all-live).
fun z532(bm: &mut vector<u8>, i: u64) {
  let byte = i / 8;
...
public fun set_lattice_commitment_for_testing(world: &mut World, zx: u32, zy: u32, team_bound: u64) {
  ...
  let wid = object::id(world);
  let (sids, tpls, xs, zs, sizes, gseeds) = zone_comp::z45(world, zx, zy, seed, team_bound);
...
public fun mob_bitmap_for_testing(world: &World, zx: u32, zy: u32): vector<u8> { z107(world, zx, zy).mob_bitmap }
