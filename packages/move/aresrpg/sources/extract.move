...
///     `extension::y33` (`NS_CHARACTER_EQUIPMENT` namespace — S-46: `public(package)` is the gate, not a cap). `unequip` reverses it, returning a `LockPledge` that FORCES a
...
/// stays confirmable-with-no-receipts forever. Neither field is ever exposed: only `y115` (this module)
...
public fun extract_for_equip(
  ...
  let item = y115(policy, kiosk, personal_cap, item_id, ctx);
...
/// goes through the `public(package)` `extension::y33` under the `NS_CHARACTER_EQUIPMENT` namespace —
...
/// inside `extension::y35`.
public fun unequip(
  character: &mut Character,
  key: ID,
  version: &Version,
): (Item, LockPledge) {
  let item: Item = extension::y35(extension::y42(), character, key, version);
...
/// pledge returned by `split`. The child is then immediately re-extracted through `y115`. Thus EVERY
...
public fun extract_one_for_burn(
  ...
  version.assert_enabled();
  let mut remainder = y115(policy, kiosk, personal_cap, item_id, ctx);
  if (item::amount(&remainder) == 1) {
    ...
    item::lock_in_kiosk(item::y55(remainder_id), remainder, kiosk, owner_cap, &policy.policy);
    item::lock_in_kiosk(unit_lock, unit, kiosk, owner_cap, &policy.policy);
    let unit = y115(policy, kiosk, personal_cap, unit_id, ctx);
...
public fun merge_locked_stacks(
  ...
  let mut target = y115(policy, kiosk, personal_cap, target_id, ctx);
  let source = y115(policy, kiosk, personal_cap, source_id, ctx);
...
public fun split_locked_stack(
  ...
  version.assert_enabled();
  let mut source = y115(extract_policy, kiosk, personal_cap, item_id, ctx);
  let (child, child_pledge) = item::split(&mut source, amount, ctx);
  let source_id = object::id(&source);
  let child_id = object::id(&child);
  let owner_cap = personal_kiosk::borrow(personal_cap);

  item::lock_in_kiosk(item::y55(source_id), source, kiosk, owner_cap, marketplace_policy);
