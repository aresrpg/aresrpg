...
public(package) fun z33(): TypeName { type_name::with_defining_ids<FightBrand>() }

#[test_only]
/// Test twin of `z33` for SIBLING suites (the dungeon engine-bridge fabricates a branded `FightOutcome`).
/// Test builds only, stripped from every publish.
public fun brand_type_for_testing(): TypeName { z33() }
...
public fun create(
  ...
  let (creator, creator_lines) = z73(kiosk, pkcap, t_character, raised_spell_ids, config, clock.timestamp_ms());
  z72(kiosk, pkcap, t_character, version);
  ...
  engine::create_round(
    ...
    is_public, party_id, false, &mob_template::z507(mob_tmpl), group_size, group_seed,
...
/// ADD the next committed member's template. `z507` is the same mirror `create` uses, so a member of a mixed
...
public fun add_member(build: &mut GroupBuild, mob_tmpl: &MobTemplate) {
  engine::add_member(build, mob_template::template_id(mob_tmpl), &mob_template::z507(mob_tmpl));
}
...
public fun release_group(
  ...
  config.assert_enabled();
  version.assert_enabled();
  assert!(settlement::brand(outcome) == z33(), EWrongBrand);
  ...
  zones::z50(world, zx, zy, index, spawn_id, x, z);
}
...
public fun join(
  ...
  config.assert_domain(config::domain_fight()); // S-46 kill-switch bit
  version.assert_enabled();
  let (joiner, joiner_lines) = z73(kiosk, pkcap, character_id, raised_spell_ids, config, clock.timestamp_ms());
  z72(kiosk, pkcap, character_id, version); // PvM — unfinished business
...
public(package) fun z34(
  ...
  let (creator, creator_lines) = z73(kiosk, pkcap, character_id, raised_spell_ids, config, clock.timestamp_ms());
  z72(kiosk, pkcap, character_id, version); // dungeon fights are PvM — the mark applies
  ...
  let group_seed = z520(scope) ^ nonce;
  engine::create(
    FightBrand {}, registry, scope, nonce, world_seed, anchor_x, anchor_z, clock.timestamp_ms(),
    false, option::none(), true, &mob_template::z507(mob_tmpl), group_size, group_seed,
...
public(package) fun z35(
  ...
  let (creator, creator_lines) = z73(kiosk, pkcap, character_id, vector[], config, clock.timestamp_ms());
  z72(kiosk, pkcap, character_id, version); // PvM ambush — the unfinished-business mark applies
  engine::create(
    FightBrand {}, registry, world_id, spawn_id, world_seed, anchor_x, anchor_z, clock.timestamp_ms(),
    false, option::none(), false, &mob_template::z507(protector_tmpl), group_size, group_seed,
...
public(package) fun z36(
  ...
  config.assert_enabled();
  version.assert_enabled();
  z72(kiosk, pkcap, character_id, version); // dungeon fights are PvM — the unfinished-business mark applies
  let (joiner, joiner_lines) = z73(kiosk, pkcap, character_id, raised_spell_ids, config, clock.timestamp_ms());
...
/// `z34` verbatim. `config` carries the pin. FightBrand stays core-private inside the delegate.
public fun create_dungeon_fight_brand<W: drop>(
  ...
  z34(registry, scope, nonce, world_seed, anchor_x, anchor_z, kiosk, pkcap, character_id, raised_spell_ids, mob_tmpl, group_size, config, version, engine_version, clock, ctx);
}
...
/// `group_seed` is derived the same way `z34` derives it (deterministic + public: rooms are
...
public fun open_room_group_brand<W: drop>(
  ...
  engine::open_group(
    ...
    false, option::none(), true, z520(scope) ^ nonce, progress, roster, creator, creator_lines,
    dial_snapshot(config), engine_version,
  )
}

/// BRAND TWIN (2026-07-13 dungeon split): the vouched-join door for the PINNED dungeon sibling (a party member
/// joins a room fight). Asserts the pin then delegates to `z36` verbatim.
public fun join_vouched_brand<W: drop>(
  ...
  config.assert_dungeon_brand<W>();
  z36(fight, registry, kiosk, pkcap, character_id, raised_spell_ids, config, version, engine_version, clock, ctx);
}
...
fun z72(kiosk: &mut Kiosk, pkcap: &PersonalKioskCap, character_id: ID, version: &Version) {
...
public fun combat_snapshot(
  ...
  let (c, _lines) = z73(kiosk, pkcap, character_id, raised_spell_ids, config, clock.timestamp_ms());
  c
}
...
fun z73(kiosk: &Kiosk, pkcap: &PersonalKioskCap, character_id: ID, raised_spell_ids: vector<ID>, config: &GameConfig, now_ms: u64): (Combatant, vector<WeaponLine>) {
  ...
  let affinity = weapon_family.is_some() && weapon_family == equipment::z13(class);
  ...
  let raw_lines = equipment::z14(character);
...
public(package) fun mark(character: &mut Character, version: &Version) {
  ...
  if (extension::z28(character, ns, DirtyKey {})) {
    let slot: &mut u64 = extension::z23(ns, character, DirtyKey {}, version);
    *slot = *slot + 1;
  } else {
    extension::z22(ns, character, DirtyKey {}, 1u64, version);
  };
}
...
public(package) fun clear(character: &mut Character, version: &Version) {
  let ns = extension::z30();
  assert!(extension::z28(character, ns, DirtyKey {}), ENotMarked);
  let remaining = {
    let slot: &mut u64 = extension::z23(ns, character, DirtyKey {}, version);
    *slot = *slot - 1;
    *slot
  };
  if (remaining == 0) { let _: u64 = extension::z24(ns, character, DirtyKey {}, version); };
}
...
public fun pending_obligations(c: &Character): u64 {
  let ns = extension::z30();
  if (extension::z28(c, ns, DirtyKey {})) {
    *extension::z29<DirtyKey, u64>(c, ns, DirtyKey {})
  } else 0
}
