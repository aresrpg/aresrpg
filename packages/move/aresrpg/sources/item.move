...
fun init(otw: ITEM, ctx: &mut TxContext) {
  ...
  let item_values = vector[
    utf8(b"{name}"),
    utf8(b"https://assets.aresrpg.world/items/{item_type}.png"), // owner ruling 2026-07-23 "item_type = the image": ABSOLUTE (external explorers have no app host) keyed by `{item_type}` — THE per-variant art slug (the seed's uniqueness gate makes each item_type unique; the old slot-word sharing was dead-lineage data)
    utf8(b"{description}"),
