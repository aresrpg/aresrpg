...
/// ONE gear-mint door (`extension::z505`, which ROLLS the template's [min,max] ranges off the seed this call
...
/// roll → z503 → kiosk-lock all happen INSIDE the one call — so each is the TERMINAL command with no result to
...
/// item. See `z89` for the enforced order.
entry fun buy(
  sale: &mut Sale,
  ...
  z89(sale, template, 1, payment, kiosk, pkcap, policy, &mut generator, clock, version, ctx);
}
...
entry fun buy_many(
  ...
  config.assert_domain(aresrpg::config::domain_market()); // S-46 kill-switch bit
  let mut generator = random::new_generator(r, ctx);
  z89(sale, template, quantity, payment, kiosk, pkcap, policy, &mut generator, clock, version, ctx);
}
...
fun z89(
  ...
  if (item::is_stackable_category(item::template_category(template))) {
    ...
    let (item, pledge) = item::z38(template, quantity, ctx);
    ...
    let mut i = 0;
    while (i < quantity) {
      let (item, pledge) = extension::z505(template, option::some(generator.generate_u64()), version, ctx);
      event::emit(SaleBought {
        sale: object::id(sale),
        template: sale.template,
        buyer,
...
#[test_only]
/// Drive `z89` for ONE item through a deterministic generator (the `entry`'s real `&Random` path is
...
#[test_only]
/// Drive `z89` for `quantity` items through a deterministic generator (the pack path).
