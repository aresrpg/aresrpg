...
// combat-snapshot seam maps a character's class string to its `ClassRow`). Order MUST match `z58`.
...
public struct GameConfig has key {
  ...
  // bridge doors (z34/z36). Both ship `none` (doors CLOSED until the ceremony pins).
...
public fun set_class_base_hp(cap: &AdminCap, config: &mut GameConfig, class_id: u64, value: u64, version: &Version, ctx: &TxContext) {
  cap.verify(ctx);
  version.assert_latest();
  let row = z510(config, class_id);
  row.base_hp = clamp(value, HP_MIN, HP_MAX);
  z511(config, class_id);
}
...
fun z510(config: &mut GameConfig, class_id: u64): &mut ClassRow {
  assert!(class_id < CLASS_COUNT, EBadClass);
  &mut config.classes[class_id]
}

// name shortened 2026-07-27: aresrpg at Sui object-size ceiling (republish restructure); see the growth row
fun z511(config: &GameConfig, class_id: u64) {
