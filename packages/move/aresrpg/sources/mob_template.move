...
/// mirrors it into the spec at the core fight doors (`z507` — resistances stored CENTERED here are DECENTERED
...
/// `z507` decodes them). `key` only — shared.
...
/// + CENTERED elemental resistances, the mob convention `z507` decodes) and xp_reward — in ONE atomic call
...
public(package) fun z507(self: &MobTemplate): MobSpec {
  mob::new_mob_spec(
...
/// The stored `Stats` block (resistances CENTERED at 32768 — the mob convention; `z507` decenters). Free
