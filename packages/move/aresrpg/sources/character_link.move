...
/// Progression fight-writes (`z10` / `z11` / `heal_hp`) are `public(package)` — the fight
...
fun z503<K: copy + drop + store, V: drop + store>(character: &mut Character, key: K, value: V, version: &Version) {
  let ns = extension::z33();
  if (extension::z29(character, ns, key)) {
    let slot: &mut V = extension::z24(ns, character, key, version);
    *slot = value;
  } else {
    extension::z23(ns, character, key, value, version);
  };
}

/// Its `+=` twin for u64 counters: add-or-create, returning the new total.
fun z504<K: copy + drop + store>(character: &mut Character, key: K, delta: u64, version: &Version): u64 {
  let ns = extension::z33();
  if (extension::z29(character, ns, key)) {
    let slot: &mut u64 = extension::z24(ns, character, key, version);
    *slot = *slot + delta;
    *slot
  } else {
    extension::z23(ns, character, key, delta, version);
    delta
  }
}
...
public(package) fun z1(character: &mut Character, world: ID, version: &Version) {
  dungeon_lock::z13(character);
  z503(character, WorldFieldKey {}, world, version);
}
...
public fun exit_dungeon_brand<W: drop>(
  ...
  extension::z26(
    extension::z33(), character, WorldFieldKey {}, world, version,
  );
}
...
public(package) fun z2(character: &mut Character, world: ID, cp: Checkpoint, version: &Version) {
  z503(character, CheckpointKey { world }, cp, version);
}
...
public(package) fun add_job_xp(character: &mut Character, job: u8, delta: u64, version: &Version): u64 {
  z504(character, JobXpKey { job }, delta, version)
}
...
public(package) fun z3(character: &mut Character, spell: ID, level: u8, version: &Version) {
  z503(character, SpellLevelKey { spell }, level, version);
}
...
public(package) fun z4(character: &mut Character, delta: u64, version: &Version) {
  z504(character, SpellPointsSpentKey {}, delta, version);
}
...
public(package) fun z5(character: &mut Character, delta: u64, version: &Version) {
  z504(character, StatPointsSpentKey {}, delta, version);
}
...
public(package) fun z6(character: &mut Character, stat: u8, delta: u64, version: &Version): u64 {
  z504(character, StatAllocKey { stat }, delta, version)
}
...
/// kiosk in the SAME call. The `LockPledge` hot potato forces the lock (no address delivery). `z20`
...
public(package) fun z7(template: &ItemTemplate, quantity: u64, version: &Version, kiosk: &mut Kiosk, owner_cap: &KioskOwnerCap, policy: &TransferPolicy<Item>, ctx: &mut TxContext) {
  let (item, pledge) = extension::z20(template, quantity, version, ctx);
...
public(package) fun z8(template: &ItemTemplate, units: u64, item_id: ID, kiosk: &mut Kiosk, pkcap: &PersonalKioskCap, xpolicy: &ItemExtractPolicy, market_policy: &TransferPolicy<Item>, version: &Version, ctx: &mut TxContext): ID {
  ...
  if (remainder >= 1) {
    let (stack, lock_pledge) = extension::z20(template, remainder, version, ctx);
...
public(package) fun mint_and_lock_output(template: &ItemTemplate, quantity: u64, version: &Version, kiosk: &mut Kiosk, owner_cap: &KioskOwnerCap, policy: &TransferPolicy<Item>, ctx: &mut TxContext): ID {
  let (item, pledge) = if (item::is_stackable_category(item::template_category(template))) {
    extension::z20(template, quantity, version, ctx)
  } else {
    assert!(quantity == 1, ENonStackableQtyGtOne);
    extension::z502(template, version, ctx)
  };
...
public(package) fun z9(pet: &mut Item, delta: u64, version: &Version) {
  ...
  let key = PetPowerKey {};
  if (extension::z27(pet, ns, key)) {
    let slot: &mut u64 = extension::z22(ns, pet, key, version);
    *slot = *slot + delta;
  } else {
    extension::z21(ns, pet, key, delta, version);
  };
}
...
public(package) fun z10(config: &GameConfig, character: &mut Character, xp: u64, version: &Version) {
  ...
  if (extension::z29(character, ns, key)) {
    let slot: &mut Progression = extension::z24(ns, character, key, version);
    ...
    let hp = progression::max_hp(config::class_row(config, z53(character)), (level as u64), stat_allocated(character, STAT_VITALITY));
    extension::z23(ns, character, key, Progression { xp: new_xp, level, hp, hp_updated_ms: 0 }, version);
  };
}
...
public(package) fun z11(character: &mut Character, hp: u64, now_ms: u64, version: &Version) {
  ...
  if (extension::z29(character, ns, key)) {
    ...
    extension::z23(ns, character, key, Progression { xp: genesis, level, hp, hp_updated_ms: now_ms }, version);
  };
}
...
public(package) fun heal_hp(config: &GameConfig, character: &mut Character, amount: u64, now_ms: u64, version: &Version) {
  ...
  let max_hp = progression::max_hp(config::class_row(config, z53(character)), level, stat_allocated(character, STAT_VITALITY));
  let slot: &mut Progression = extension::z24(ns, character, ProgressionKey {}, version);
...
public fun combat_stats(character: &Character, config: &GameConfig): (String, u64, u64, u64, u64, u64) {
  z51(character, config, option::none())
}
...
public fun combat_stats_settled(character: &Character, config: &GameConfig, now_ms: u64): (String, u64, u64, u64, u64, u64) {
  z51(character, config, option::some(now_ms))
}
...
fun z51(character: &Character, config: &GameConfig, settle_at_ms: Option<u64>): (String, u64, u64, u64, u64, u64) {
  let row = config::class_row(config, z53(character));
  let level = level(character);
...
public fun world_field(character: &Character): Option<ID> {
  ...
  if (extension::z29(character, ns, key)) {
    option::some(*extension::z30<WorldFieldKey, ID>(character, ns, key))
  } else {
    option::none()
  }
}
...
public fun has_checkpoint(character: &Character, world: ID): bool {
  extension::z29(character, extension::z33(), CheckpointKey { world })
}
...
public fun checkpoint(character: &Character, world: ID): Checkpoint {
  *extension::z30<CheckpointKey, Checkpoint>(character, extension::z33(), CheckpointKey { world })
}
...
public fun level(character: &Character): u64 {
  if (has_progression(character)) (z52(character).level as u64)
...
public fun has_progression(character: &Character): bool {
  extension::z29(character, extension::z31(), ProgressionKey {})
}
...
public fun progression_hp(character: &Character): u64 { z52(character).hp }
...
public fun pet_power(pet: &Item): u64 {
  let ns = extension::ns_item();
  let key = PetPowerKey {};
  if (extension::z27(pet, ns, key)) *extension::z28<PetPowerKey, u64>(pet, ns, key)
  else 0
}
...
public fun job_xp(character: &Character, job: u8): u64 {
  ...
  if (extension::z29(character, ns, key)) {
    *extension::z30<JobXpKey, u64>(character, ns, key)
  } else {
    0
  }
}
...
public fun spell_level(character: &Character, spell: ID): u8 {
  ...
  if (extension::z29(character, ns, key)) {
    *extension::z30<SpellLevelKey, u8>(character, ns, key)
  } else {
    1
  }
}
...
public fun spell_points_spent(character: &Character): u64 {
  ...
  if (extension::z29(character, ns, key)) {
    *extension::z30<SpellPointsSpentKey, u64>(character, ns, key)
  } else {
    0
  }
}
...
public fun stat_allocated(character: &Character, stat: u8): u64 {
  ...
  if (extension::z29(character, ns, key)) {
    *extension::z30<StatAllocKey, u64>(character, ns, key)
  } else {
    0
  }
}
...
public fun stat_points_spent(character: &Character): u64 {
  ...
  if (extension::z29(character, ns, key)) {
    *extension::z30<StatPointsSpentKey, u64>(character, ns, key)
  } else {
    0
  }
}
...
// name shortened 2026-07-27: aresrpg at Sui object-size ceiling (ceremony leg-2); see the growth row
fun z52(character: &Character): Progression {
  *extension::z30<ProgressionKey, Progression>(character, extension::z31(), ProgressionKey {})
}

// name shortened 2026-07-27: aresrpg at Sui object-size ceiling (ceremony leg-2); see the growth row
/// Resolve the character's class SLUG to its GameConfig class id, aborting `EUnknownClass` if it is not a §3 class.
fun z53(character: &Character): u64 {
