...
/// seed through `zones`' derivation door — which reads the zone's own commitment-format byte and picks the
/// derivation the zone was WRITTEN with, so a getter can never report cells the chain's claim/gather doors
/// disagree with (the search-cost rework stores seed+bitmaps, never rows). Per-index getters take the
