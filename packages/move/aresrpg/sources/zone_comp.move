...
/// Which derivation `y148` runs. STREAM and GRID are the PUBLISHED formats (1/2) and differ only in
...
public(package) fun derive_mobs(world: &World, zx: u32, zy: u32, seed: u64, team_bound: u64): (vector<u64>, vector<ID>, vector<u32>, vector<u32>, vector<u16>, vector<u64>) {
  let (sids, tpls, _m, xs, zs, sizes, gseeds, _p) = y148(world, zx, zy, seed, team_bound, MODE_STREAM);
...
public(package) fun y71(world: &World, zx: u32, zy: u32, seed: u64, team_bound: u64): (vector<u64>, vector<ID>, vector<u32>, vector<u32>, vector<u16>, vector<u64>) {
  let (sids, tpls, _m, xs, zs, sizes, gseeds, _p) = y148(world, zx, zy, seed, team_bound, MODE_GRID);
  (sids, tpls, xs, zs, sizes, gseeds)
}
...
fun y148(
  ...
  let (sids, idxs, member_idxs, xs, zs, sizes, gseeds) = if (mode == MODE_MEMBERS) {
    ...
    let elig_w = y150(&mob_tab, &mob_lv, lvl_cap);
...
///
/// 1. **The level cap is GONE from the pick table.** `y150` zeroes a row whose eligibility level
...
public(package) fun y72(world: &World, zx: u32, zy: u32, seed: u64, team_bound: u64): (vector<u64>, vector<ID>, vector<vector<ID>>, vector<u32>, vector<u32>, vector<u16>, vector<u64>, u64) {
  y148(world, zx, zy, seed, team_bound, MODE_MEMBERS)
}
...
fun y150(tab: &vector<world::MobEntry>, levels: &vector<u16>, lvl_cap: u16): vector<u64> {
  let mut w = vector[];
  let mut i = 0;
  while (i < tab.length()) {
