...
/// passed defender `MobTemplate`, INTRA-call through `fight::z35` (the engine composes from a
...
/// The yield now mints as ONE stacked `Item` of amount `quantity` through `character_link::z7`
/// (`extension::z19` — the resource category STACKS), so the `quantity` event value and the minted
...
const ETierLocked: u64 = 106; // the character's job level is below the resource tier's z502 level
...
const XP_BAND_WIDTH: u64 = 10; // one tier band = 10 job levels (tiers z502 every 10 levels)
...
entry fun gather(
  ...
  z74(world, kiosk, pkcap, character_id, zx, zy, node_index, template, rare_template, policy, registry, protector_template, engine_version, config, version, clock, &mut gen, ctx);
}
...
fun z74(
  ...
  let (nx, nz, njob, ntier, ntemplate) = zones::z51(world, zx, zy, node_index);
  ...
  let quantity = z75(job_level, required);
  let gained_xp = z76(ntier, job_level);
  ...
  let ambush_rolled = z77(world::protector_bp(world), gen);
  ...
  zones::z52(world, zx, zy, node_index);
  {
    let character = kiosk.borrow_mut(owner_cap, character_id);
    let pet = equipment::pet_equipped(character);
    character_link::z2(character, wid, world::z44(nx, nz, now, pet), version);
  ...
  character_link::z7(template, quantity, version, kiosk, owner_cap, policy, ctx);
  ...
  z79(world, ntemplate, rare_template, gen, gatherer, wid, version, kiosk, owner_cap, policy, ctx);
  ...
  let protector_spawn_id = if (protector && z78(kiosk, owner_cap, character_id)) {
    ...
    fight::z35(registry, wid, spawn_id, world::seed(world), nx, nz, group_seed, kiosk, pkcap, character_id, protector_template, 1, config, version, engine_version, clock, ctx);
    spawn_id
  } else 0;
...
fun z75(job_level: u64, required: u64): u64 {
...
fun z76(tier: u8, job_level: u64): u64 {
...
fun z77(bp: u64, gen: &mut RandomGenerator): bool {
...
fun z78(kiosk: &Kiosk, owner_cap: &KioskOwnerCap, character_id: ID): bool {
...
/// (lead ruling). Then ONE `RARE_BP` draw from the SAME rng; on a hit, mint via `z521`. The extra draw exists
/// ONLY for rare-linked resources; the DF link stays the authority — a richer substitute template can never mint.
fun z79(world: &World, base_tid: ID, rare_template: &ItemTemplate, gen: &mut RandomGenerator, gatherer: address, wid: ID, version: &Version, kiosk: &mut Kiosk, owner_cap: &KioskOwnerCap, policy: &TransferPolicy<Item>, ctx: &mut TxContext) {
  ...
  if (!z77(RARE_BP, gen)) return;
  z521(rare_template, base_tid, gatherer, wid, version, kiosk, owner_cap, policy, ctx);
}

// name shortened 2026-07-27: aresrpg at Sui object-size ceiling (republish restructure); see the growth row
/// Mint ONE unit of the (already-verified) linked rare variant through the SAME cap-gated door + kiosk-lock as the
/// normal yield, then emit `RareGathered`. Identity was asserted in `z79` BEFORE the draw — the hit path
/// itself never aborts on template identity, so a won jackpot always mints.
fun z521(rare_template: &ItemTemplate, base_tid: ID, gatherer: address, wid: ID, version: &Version, kiosk: &mut Kiosk, owner_cap: &KioskOwnerCap, policy: &TransferPolicy<Item>, ctx: &mut TxContext) {
  character_link::z7(rare_template, 1, version, kiosk, owner_cap, policy, ctx);
...
#[test_only]
public fun test_yield(job_level: u64, required: u64): u64 { z75(job_level, required) }

#[test_only]
public fun test_job_xp(tier: u8, job_level: u64): u64 { z76(tier, job_level) }

#[test_only]
public fun test_protector_fires(bp: u64, gen: &mut RandomGenerator): bool { z77(bp, gen) }
...
/// `RareGathered` exactly as a live hit does. Template identity is `z79`'s PRE-roll gate, not the mint's.
