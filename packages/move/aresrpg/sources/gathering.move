...
/// The yield now mints as ONE stacked `Item` of amount `quantity` through `character_link::y8`
/// (`extension::y30` — the resource category STACKS), so the `quantity` event value and the minted
...
const ETierLocked: u64 = 106; // the character's job level is below the resource tier's y16 level
...
const XP_BAND_WIDTH: u64 = 10; // one tier band = 10 job levels (tiers y16 every 10 levels)
...
fun y119(
  ...
  let (nx, nz, njob, ntier, ntemplate) = zones::y77(world, zx, zy, node_index);
  ...
  let quantity = y120(job_level, required);
  let gained_xp = y121(ntier, job_level);
  ...
  let ambush_rolled = y122(world::protector_bp(world), gen);
  ...
  zones::y78(world, zx, zy, node_index);
  {
    let character = kiosk.borrow_mut(owner_cap, character_id);
    let pet = equipment::pet_equipped(character);
    character_link::y2(character, wid, world::y70(nx, nz, now, pet), version);
    character_link::y3(character, njob, gained_xp, version);
  };

  // 8) mint the whole yield as ONE stacked resource item through the cap-gated door + lock into the gatherer's kiosk
  character_link::y8(template, quantity, version, kiosk, owner_cap, policy, ctx);
  ...
  y124(world, ntemplate, rare_template, gen, gatherer, wid, version, kiosk, owner_cap, policy, ctx);
  ...
  let protector_spawn_id = if (protector && y123(kiosk, owner_cap, character_id)) {
...
fun y123(kiosk: &Kiosk, owner_cap: &KioskOwnerCap, character_id: ID): bool {
...
/// (lead ruling). Then ONE `RARE_BP` draw from the SAME rng; on a hit, mint via `y125`. The extra draw exists
/// ONLY for rare-linked resources; the DF link stays the authority — a richer substitute template can never mint.
fun y124(world: &World, base_tid: ID, rare_template: &ItemTemplate, gen: &mut RandomGenerator, gatherer: address, wid: ID, version: &Version, kiosk: &mut Kiosk, owner_cap: &KioskOwnerCap, policy: &TransferPolicy<Item>, ctx: &mut TxContext) {
...
fun y125(rare_template: &ItemTemplate, base_tid: ID, gatherer: address, wid: ID, version: &Version, kiosk: &mut Kiosk, owner_cap: &KioskOwnerCap, policy: &TransferPolicy<Item>, ctx: &mut TxContext) {
  character_link::y8(rare_template, 1, version, kiosk, owner_cap, policy, ctx);
...
#[test_only]
public fun test_yield(job_level: u64, required: u64): u64 { y120(job_level, required) }

#[test_only]
public fun test_job_xp(tier: u8, job_level: u64): u64 { y121(tier, job_level) }

#[test_only]
public fun test_protector_fires(bp: u64, gen: &mut RandomGenerator): bool { y122(bp, gen) }
...
/// `RareGathered` exactly as a live hit does. Template identity is `y124`'s PRE-roll gate, not the mint's.
