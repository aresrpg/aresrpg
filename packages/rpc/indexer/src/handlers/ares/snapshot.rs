...
/// EVERY `aresrpg` origin whose MobTemplate bodies this indexer can walk, paired with the
/// `spell_effect::Effect` width that origin's templates embed. The lineage — not a boundary
/// constant with an open-ended default (issue #1315 finding 9): the republish that wraps `World`
/// in `Versioned` mints ANOTHER origin whose Effects are still 33 bytes, and a default-25 would
/// have misaligned every byte past `spells` for the whole new universe, silently.
///
...
const ARES_ORIGIN_EFFECT_BYTES: &[(&str, usize)] = &[
    ...
    ("0x4217b46f8dfe7c1ccc6a5e1c37e012a53bf25b07e0edb228a3c5a1575eeb2b06", 25),
    ...
    ("0xc05c8d9f65e4ee0a6dc9463c8c90341085daa062184043e1a6fd75bccd6842e1", 33),
    ("0x8781c9c8e9f87113536a6926e441b359a7bc24fda9a80a2b9b225a8d51d0da98", 33),
    (FRESH_ARESRPG_ORIGIN, 33),
];
...
/// that origin is not in [`ARES_ORIGIN_EFFECT_BYTES`] — the fail-closed answer the loot walk must
/// obey rather than guess. ONE home for the origin→width decision; `skip_effect_vec`'s caller
/// chain never re-derives it.
fn effect_byte_width(package: &str) -> Option<usize> {
    ARES_ORIGIN_EFFECT_BYTES
        .iter()
        .find(|(origin, _)| *origin == package)
        .map(|(_, width)| *width)
}
