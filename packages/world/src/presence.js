...
// Effects live at the edges: the p2p transport (frontend `src/p2p/lobby-room.js`) dispatches typed inputs
// and reads the atom to send; identity resolution is an effect REQUEST (the adapter reads the chain and answers
// through the door).
//
// ONE ROSTER (#1698). `peers` is the only table of who is here, because JOINING THE ROOM IS THE ANNOUNCEMENT
// (docs/REALTIME.md lane 2). The server-observed `online` map that briefly lived beside it is gone with the
// registry that fed it: that registry had the read layer writing redis on a client's say-so — law 1 (only the
// indexer writes redis) and law 2 (zero client→server writes) in one stroke — and two presence homes disagreed
// in the exact way a second home always does (a friend standing still was "online" in one and absent from the
// other, so fast travel refused a same-world friend as unreachable).
...
// CLOSED (#1698, the architecture call the #1641 note deferred): PEER_HEARTBEAT_MS has a producer again. Under
// the courier a pose was POSTed only on an actual cell/facing change, so a player who stood still lapsed out of
// its TTL rows and vanished from every later joiner's snapshot — the "keep-alive or longer TTL?" question. The
// answer was neither: the transport re-emits my last cell on this cadence (lobby-room.js `_heartbeat`), so a
// stationary peer is provably alive and `tick` expires only the genuinely silent.
...
export const PEER_HEARTBEAT_MS = 7_000 // the re-emit cadence for my own pose (lobby-room.js drives it); a backgrounded tab's real send gap is bounded by the browser's throttle floor, not this number — which is exactly why PEER_EXPIRY_MS is sized off that floor and not off this cadence
...
// `link_status` / `link_error`. The transport edge (frontend `src/p2p/lobby-room.js`) owns the room, its
// sockets and its finite retry budget; it reports what the link IS through this one input, and the UI reads
// the atom. Before #1641 the edge only logged its status, so the chip read "P2P idle" against a fully live
// link — a status with no writer is a lie with a UI, and the fix is that this input has exactly one writer.
