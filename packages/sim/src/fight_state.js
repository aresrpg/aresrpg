...
/**
 * Sum of the ap/mp pool modifiers active for the upcoming turn. Timed rows now age at their owner's preceding
 * turn end, exactly like Move, so every row still present at begin-turn contributes to the refill — including a
 * row with one turn remaining, which expires only after this final usable turn.
 * @param {FightEntity} entity
 * @param {'ap'|'mp'} key
 * @returns {number}
 */
