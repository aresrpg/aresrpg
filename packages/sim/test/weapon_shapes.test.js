...
describe('§387 weapon shape cell-set derivation (sim twin)', () => {
  test('1-CELL (POINT) hits only the aimed cell', () => {
    expect(cells(SHAPE_POINT, 0, { x: 6, y: 5 }, { x: 5, y: 5 })).toEqual(['6,5'])
  })

  describe('2-INLINE (LINE size 1) — aimed cell + the next cell along the strike direction', () => {
    test('strike EAST', () => {
      // attacker (5,5) → target (6,5): beyond = (7,5)
      expect(cells(SHAPE_LINE, 1, { x: 6, y: 5 }, { x: 5, y: 5 })).toEqual(set_of('6,5', '7,5'))
    })
    test('strike WEST', () => {
      expect(cells(SHAPE_LINE, 1, { x: 6, y: 5 }, { x: 7, y: 5 })).toEqual(set_of('6,5', '5,5'))
    })
    test('strike SOUTH (+y)', () => {
      expect(cells(SHAPE_LINE, 1, { x: 5, y: 6 }, { x: 5, y: 5 })).toEqual(set_of('5,6', '5,7'))
    })
    test('strike NORTH (-y)', () => {
      expect(cells(SHAPE_LINE, 1, { x: 5, y: 6 }, { x: 5, y: 7 })).toEqual(set_of('5,6', '5,5'))
    })
  })
  ...
  describe('3-FRONT-ARC (TBAR size 1) — aimed cell + its two PERPENDICULAR neighbours', () => {
    test('horizontal strike → vertical bar', () => {
      ...
      expect(cells(SHAPE_TBAR, 1, { x: 6, y: 5 }, { x: 5, y: 5 })).toEqual(set_of('6,5', '6,4', '6,6'))
    })
    test('vertical strike → horizontal bar', () => {
      // attacker (5,5) → target (5,6) [south]: perpendicular = east/west of target
      expect(cells(SHAPE_TBAR, 1, { x: 5, y: 6 }, { x: 5, y: 5 })).toEqual(set_of('5,6', '4,6', '6,6'))
    })
  })
  ...
  describe('orientation edge cases', () => {
    test('diagonal target — the dominant axis wins ties (x), matching push/displacement', () => {
      ...
      expect(cells(SHAPE_TBAR, 1, { x: 6, y: 6 }, { x: 5, y: 5 })).toEqual(set_of('6,6', '6,5', '6,7'))
    })
    test('board EDGE drops the off-grid cells of the shape', () => {
      ...
      expect(cells(SHAPE_PODIUM, 1, { x: 3, y: 0 }, { x: 3, y: 1 })).toEqual(set_of('3,0', '2,0', '4,0'))
    })
  })
})
