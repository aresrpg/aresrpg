...
import senshi_spells from '../../../seed/mainnet/spells/senshi.json'
...
import { arena, cast, fighter, spell_of, state_of } from './missing_effect_helpers.js'

const senshi_vault_raw = senshi_spells.find(s => s.id === 'senshi_vault')
...
describe('sim contract: senshi teleport + push displacement', () => {
  test('senshi teleport (kind 14) moves the caster onto the target cell', () => {
    ...
    const teleport = spell_of('senshi_vault', [
      { kind: K_TELEPORT, value: 2, target_filter: TF_ONLY_CASTER, chance: 100 },
    ])
  ...
  test('REAL senshi_vault template teleports the caster through reduce()', () => {
    const caster = fighter('p0', { x: 2, y: 4 }, true)
    const enemy = fighter('m0', { x: 6, y: 4 }, false)
    const state = state_of([caster], [enemy])
    ...
    const result = reduce(state, command, { spell_templates: templates, arena })
