...
const ALL_KINDS = Object.fromEntries(
  Object.entries(SE).filter(([name, v]) => name.startsWith('K_') && typeof v === 'number'),
)
const ALL_KIND_VALUES = [...new Set(Object.values(ALL_KINDS))].sort((a, b) => a - b)
...
const layouts = {
  ...
  point_blank() {
    const caster = fighter('p0', cell(2, 2), true, { health_max: 200, health: 200 })
    const enemy = fighter('m0', cell(3, 2), false)
    return { state: state_of([caster], [enemy]), caster, enemy, self: caster.cell, target: enemy.cell }
  },
  ...
  wounded_ally() {
    ...
    const ally = fighter('p1', cell(1, 2), true, { health: 30, health_max: 100 })
    const enemy = fighter('m0', cell(6, 2), false)
    return { state: state_of([caster, ally], [enemy]), caster, ally, enemy, self: caster.cell, target: ally.cell }
  },
  // A ranged enemy with open board behind it: push / pull / teleport / geometric shapes have room to resolve.
  ranged() {
    const caster = fighter('p0', cell(2, 4), true, { health_max: 200, health: 200 })
  ...
  cluster() {
    const caster = fighter('p0', cell(4, 4), true, { health_max: 300, health: 300 })
    const mobs = [
    ...
    return { state: state_of([caster], mobs), caster, mobs, self: caster.cell, target: cell(4, 3) }
  },
}

// Cast a synthetic single-effect spell of `kind` through the REDUCER on a layout; return {events, castEvent, state}.
const cast_through_reducer = (layout_name, effect, target_key = 'target', level_effects = null) => {
  ...
  const command = { type: 'cast', entity_id: built.caster.id, spell_id, target: built[target_key] }
  ...
  const twin = reduce(rebuilt.state, { ...command, target: rebuilt[target_key] }, { spell_templates: templates, arena })
  return {
    result,
    twin,
    changed,
...
const matrix = [
  { kind: SE.K_DAMAGE, on: ['point_blank', 'ranged', 'cluster'], effects: true },
  ...
  { kind: SE.K_CASTER_DAMAGE, on: ['point_blank'], eff: { target_filter: SELF }, target: 'self', effects: true },
  { kind: SE.K_PUNISHMENT_DAMAGE, on: ['point_blank'], effects: true },
  { kind: SE.K_HEAL, on: ['wounded_ally'], eff: { target_filter: FRIEND }, target: 'target', effects: true },
  {
  ...
  { kind: SE.K_REMOVE_POINTS, on: ['point_blank'], eff: { stat: SE.POINT_MP, value: 2 }, effects: true },
  { kind: SE.K_STEAL_POINTS, on: ['point_blank'], eff: { stat: SE.POINT_AP, value: 2 } }, // pool delta → state
  { kind: SE.K_ALTER_STAT, on: ['point_blank'], eff: { stat: SE.STAT_STRENGTH, value: 20, turns: 2 }, effects: true },
  { kind: SE.K_STEAL_STAT, on: ['point_blank'], eff: { stat: SE.STAT_STRENGTH, value: 20, turns: 2 } }, // → state
  { kind: SE.K_ALTER_RESIST, on: ['point_blank'], eff: { element: 2, value: 30, turns: 2 } },
  { kind: SE.K_PUSH, on: ['ranged', 'point_blank'], eff: { value: 3, element: 255 }, effects: true },
  { kind: SE.K_PULL, on: ['ranged'], eff: { value: 2, element: 255 }, effects: true },
  { kind: SE.K_TELEPORT, on: ['ranged'], eff: { element: 255 }, target: 'empty' },
  ...
  { kind: SE.K_RESET_POSITIONS, on: ['ranged'], eff: { target_filter: ZONE, element: 255 }, target: 'self' },
  {
    kind: SE.K_PLACE_TRAP,
    on: ['ranged'],
    eff: { target_filter: ZONE, element: 255, area_shape: SE.SHAPE_CIRCLE, area_size: 1 },
    target: 'empty',
  },
  {
    kind: SE.K_PLACE_GLYPH,
    on: ['ranged'],
    eff: { target_filter: ZONE, element: 255, area_shape: SE.SHAPE_CIRCLE, area_size: 1, turns: 3, phase: SE.PHASE_START },
    target: 'empty',
  },
  { kind: SE.K_APPLY_DOT, on: ['point_blank'], eff: { value: 8, turns: 3, phase: SE.PHASE_START }, effects: true },
  { kind: SE.K_APPLY_STATE, on: ['point_blank'], eff: { value: 1, turns: 2 } },
  { kind: SE.K_REMOVE_STATE, on: ['point_blank'], eff: { value: 1 } },
  { kind: SE.K_REDUCE_DAMAGE, on: ['wounded_ally'], eff: { target_filter: FRIEND, value: 40, turns: 2 }, target: 'target' },
  { kind: SE.K_REFLECT_DAMAGE, on: ['wounded_ally'], eff: { target_filter: FRIEND, value: 30, turns: 2 }, target: 'target' },
  ...
  { kind: SE.K_RETURN_SPELL, on: ['wounded_ally'], eff: { target_filter: SELF, element: 255, turns: 2 }, target: 'self' },
  {
    kind: SE.K_GEOMETRIC_PUSH,
    on: ['cluster'],
    eff: { target_filter: ZONE, element: 255, area_shape: SE.SHAPE_CIRCLE, area_size: 2 },
    target: 'self',
  },
  { kind: SE.K_CRITICAL_FAILURE, on: ['point_blank'], eff: { value: 1, turns: 1 } },
  { kind: SE.K_DAMAGE_TO_HEAL, on: ['point_blank'], eff: { value: 1, turns: 2 } },
  { kind: SE.K_FORCED_DEATH, on: ['point_blank'], eff: { element: 255 }, effects: true },
  {
    kind: SE.K_TIMED_PAYLOAD,
    on: ['point_blank'],
    // A timed payload wraps FOLLOWING effects; give it a real linked damage effect so it normalizes + arms.
    levels: [
      eff(SE.K_TIMED_PAYLOAD, { stat: 1, turns: 2, target_filter: SELF, element: 255 }),
      eff(SE.K_DAMAGE, { value: 10 }),
    ],
    target: 'self',
  },
  { kind: SE.K_NAMED_DAMAGE_STACK, on: ['point_blank'], eff: { value: 10, turns: 2 }, effects: true },
  { kind: SE.K_STANCE, on: ['wounded_ally'], eff: { target_filter: SELF, value: 1, turns: 2 }, target: 'self' },
  {
  ...
  { kind: SE.K_EROSION, on: ['point_blank'], eff: { value: 10, turns: 2 }, effects: true },
  { kind: SE.K_DAMAGE_REDIRECT, on: ['wounded_ally'], eff: { target_filter: FRIEND, value: 1, turns: 2 }, target: 'target' },
]
...
describe('PILLAR 2a — every spell-effect kind executes deterministically through the reducer', () => {
  for (const row of matrix) {
    const kind_name = Object.keys(ALL_KINDS).find(name => ALL_KINDS[name] === row.kind) ?? `K_${row.kind}`
    for (const layout_name of row.on) {
      test(`${kind_name} · ${layout_name} · executes + deterministic`, () => {
        ...
        let probe
        try {
          probe = cast_through_reducer(layout_name, effect, row.target ?? 'target', row.levels ?? null)
        } finally {
        ...
        expect(castEvent, `${kind_name} cast was rejected by the reducer on ${layout_name}`).toBeTruthy()
    ...
    test(`AoE ${shape_name} · damage fans across the mob cluster deterministically`, () => {
      ...
      try {
        probe = cast_through_reducer(
          'cluster',
          eff(SE.K_DAMAGE, { value: 15, area_shape: shape, area_size: shape === SE.SHAPE_ALLMAP ? 0 : 3 }),
          'target',
        )
      } finally {
        layouts.cluster = orig
      }
      expect(probe.castEvent, `${shape_name} damage cast was rejected`).toBeTruthy()
      expect(JSON.stringify(probe.twin.events)).toBe(JSON.stringify(probe.result.events))
      ...
      const hits = probe.castEvent.effects.filter(e => (e.damage ?? 0) > 0).length
      if (shape !== SE.SHAPE_RING) expect(hits, `${shape_name} hit no mob in the cluster`).toBeGreaterThanOrEqual(1)
    })
  }
  ...
  test('COVERAGE — every K_* effect discriminant is exercised by the matrix', () => {
    ...
    const missing = ALL_KIND_VALUES.filter(k => !covered.has(k)).map(
      k => Object.keys(ALL_KINDS).find(name => ALL_KINDS[name] === k) ?? `K_${k}`,
    )
    expect(missing, `effect kinds shipped with NO sim-matrix row: ${missing.join(', ')}`).toEqual([])
