...
describe('zone_derive — seed-derived zone composition', () => {
  test('parity reference vectors (byte-for-byte with world_math.move derive_mob_groups)', () => {
    ...
    expect(g[0]).toEqual({ spawn_id: 4560507522876923188n, template_idx: 0, x: 510, z: 404, size: 2, group_seed: 2711362666 })
    expect(g[1]).toEqual({ spawn_id: 148739572111110642n, template_idx: 1, x: 127, z: 313, size: 6, group_seed: 4214906596 })
    expect(g[4]).toEqual({ spawn_id: 665305103150388313n, template_idx: 1, x: 213, z: 278, size: 3, group_seed: 572914111 })
    expect(g[7]).toEqual({ spawn_id: 522455783208353171n, template_idx: 1, x: 63, z: 92, size: 6, group_seed: 1370609044 })
  })
    ...
    for (let seed = 1; seed <= 200; seed++) {
      ...
      for (let i = 0; i < gs.length; i++)
        for (let j = i + 1; j < gs.length; j++) {
          ...
          if (d2 < _spacing * _spacing) violations++
        }
    }
    expect(violations).toBe(0)
    expect(worst).toBeGreaterThanOrEqual(_spacing)
  })
...
describe('zone_derive — resource cells (one-harvest / one-bit)', () => {
  ...
  test('parity reference vectors (zone_gen_tests.move::t_derive_resources_matches_js_mirror)', () => {
    ...
    expect(cells[0]).toEqual({ spawn_id: 4278267242700732727n, template_idx: 0, x: 11, z: 40 })
    expect(cells[1]).toEqual({ spawn_id: 6634652389384369540n, template_idx: 0, x: 12, z: 40 })
    expect(cells[5]).toEqual({ spawn_id: 13261909094981075068n, template_idx: 0, x: 13, z: 39 })
    expect(cells[6]).toEqual({ spawn_id: 14448448036690126015n, template_idx: 0, x: 245, z: 162 })
    expect(cells[11]).toEqual({ spawn_id: 390510390697315139n, template_idx: 0, x: 243, z: 162 })
  })

  test('non-gather rows (job > 2) land single cells (the one-bit collapse)', () => {
    const cells = derive_resources({ ...res_base, min_n: 3, max_n: 3, weights: [100], min_qty: [4], max_qty: [4], jobs: [5] })
    expect(cells).toHaveLength(3) // 3 picks -> 3 single cells, never a field, never a charge counter
    expect(cells[0]).toEqual({ spawn_id: 2975216761506653025n, template_idx: 0, x: 11, z: 40 })
    expect(cells[2]).toEqual({ spawn_id: 13191861150870521758n, template_idx: 0, x: 494, z: 395 })
  })
...
describe('zone_derive — the full derive_zone pipeline (chain twin: zone_comp.move)', () => {
  ...
  const zone = { seed: 9876543210, discovered_at_ms: 2000, mob_bitmap: [], res_bitmap: [] }
  ...
  test('rows match the Move zone_comp vectors (weights, size cap, geometry, template mapping)', () => {
    ...
    expect(mobs[0]).toEqual({
      spawn_id: '11220703129345358465', kind: 'mob', index: 0, x: 250008, z: 250195,
      template_id: '0xb0b', size: 2, spawned_at_ms: 2000, group_seed: '3875465078',
    })
    expect(mobs[2]).toMatchObject({ spawn_id: '8618570982553016694', index: 2, x: 250239, z: 250329, size: 2 })
    expect(cells[0]).toEqual({
      spawn_id: '10736692352345019500', kind: 'resource', index: 0, x: 250175, z: 250326,
      template_id: '0xwheat', remaining: 1, job: 0, tier: 1,
    })
    expect(cells[1]).toMatchObject({ spawn_id: '4596960998799914108', index: 1, x: 250267, z: 250287 })
  })
  ...
  test('consumed bits filter rows out but surviving rows KEEP their derivation index', () => {
    ...
    const rows = derive_zone({ zone: { ...zone, mob_bitmap: [0b001], res_bitmap: [0b010] }, zx: 488, zy: 488, world, team_bound: 6 })
    const mobs = rows.filter(r => r.kind === 'mob')
    const cells = rows.filter(r => r.kind === 'resource')
...
describe('zone_derive — spawn-zone-relative difficulty', () => {
  ...
  test('the existing continuous curve begins at the spawn boundary and reaches the far band', () => {
    const values = [progress(489, 488), progress(490, 488), progress(492, 488), progress(500, 488)]
