...
//
// `K_RESET_POSITIONS` (18) and `K_REMOVE_STATE` (23) are declared on both sides of the twin — `spell_effect.js`
// exports them and `spell_effect.move:44,51` mints the same numbers — and neither side has an arm for them: the
// normalizer degrades both to `UNSUPPORTED`, so a spell authored with either folds NOTHING.
//
// That fact already has two consumers, each of which TRUSTS it rather than checking it:
//   • `effect_kind_matrix.test.js` flags both rows `unsupported: true` and asserts they fold nothing;
//   • `seeded_spell_effect_conformance.test.js` quarantines both contract rows behind `skip_reason`.
