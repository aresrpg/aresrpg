...
import { checkpoint_lag, create_indexing_health, parse_indexed_checkpoint } from '../src/indexing_health.ts'
...
describe('indexing health', () => {
  ...
  test('deduplicates concurrent reads and caches one result for the heartbeat window', async () => {
    ...
    expect(await Promise.all([health(), health()])).toEqual([20, 20])
    expect(await health()).toBe(20)
