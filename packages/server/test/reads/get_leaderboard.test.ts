...
const observation = { metric: 'xp' as const, season: null, id: 7 }
...
const projection = (count: number) => {
  ...
  const state = {
    meta: JSON.stringify({ origin_epoch: 100, epoch: 130, checkpoint: 50 }),
  ...
  const call = async (command: string, ...args: readonly (string | number)[]): Promise<unknown> => {
    ...
    if (command === 'ZRANGE') return members.slice(Number(args[1]), Number(args[2]) + 1)
...
test('returns 100 addresses and an exact personal rank outside that window', async () => {
  ...
  expect(snapshot).toMatchObject({ season: 1, current_season: 1, start_epoch: 130, end_epoch: 160, epoch: 130 })
})
...
test('retries a read that crossed an indexed checkpoint', async () => {
  ...
  const call = async (command: string, ...args: readonly (string | number)[]): Promise<unknown> => {
    if (command === 'ZRANGE' && !changed) {
      changed = true
      redis.state.meta = JSON.stringify({ origin_epoch: 100, epoch: 130, checkpoint: 51 })
    }
...
test('pending writes and future seasons are unavailable rather than partial standings', async () => {
  ...
  await expect(get_leaderboard(redis, { ...observation, season: 2 }, address(1))).rejects.toThrow('future')
