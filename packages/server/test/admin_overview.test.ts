...
test('overview derives exact active and money totals from the selected tier', async () => {
  ...
  const bus = {
    analytics_hashes: async (keys: readonly string[]) =>
      keys.map((key) =>
        key === `analytics:money:day:${now_ms}`
          ? {
          ...
          : key.startsWith('analytics:characters:')
            ? {}
            : {}
      ),
  ...
  expect(result.revenue.last_30d_revenue_mist).toBe('138')
  expect(result.revenue.month_to_date_revenue_mist).toBe('138')
