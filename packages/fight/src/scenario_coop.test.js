...
import { state_hash, canonical_state } from './inputs.js'
import { committed_state } from './store.js'
import { engine_view, board_view, presenting } from './project.js'
...
describe('coop — two actors, one truth', () => {
  test('JOIN DOOR: the second seat adopts through the same snapshot input; each client resolves ITS seat', () => {
  ...
  test('R1 LOCALITY BY SEAT: one feed — bob’s turn paces as a wave for ALICE, never for bob himself', () => {
    ...
    const a = committed_state(alice.getState())
    const b = committed_state(bob.getState())
