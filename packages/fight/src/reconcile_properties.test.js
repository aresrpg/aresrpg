...
const fight_object = ({ mob_hp = 20, mob_cell = encode(5, 4), my_hp = 50, my_cell = encode(2, 2), turn_ptr = 0 } = {}) => ({
  id: FIGHT,
  status: 1,
  width: 20,
  height: 19,
  participants: [
    { owner: '0xaaa', character: CHAR, class: 'senshi', team: 0, ap: 6, mp: 3, base_ap: 6, base_mp: 3, hp: my_hp, max_hp: 50, cell: my_cell, ready: true },
  ],
...
describe('§⑤.1 monotonic-idempotence', () => {
  test('a snapshot at v ≤ canonical_version is a no-op (reduce(S, snapshot(v)) === S)', () => {
    const store = boot()
    store.getState().input({ type: 'receipt', fight_id: FIGHT, version: 6, receipt: { events: [ev('MobMoved', { idx: 0, to_cell: encode(6, 4) })] } }, 2_000)
    drain(store, 2_100)
    ...
    expect(state_hash(store.getState()), 'no field moved below its floor — the stale/equal reads are inert').toBe(before)
  })
...
describe('§⑤.2 retirement-permanence', () => {
  test('death@vD held ⇒ no snapshot sequence (vD-1 / vD / vD+3, positive hp) + stale foreign event resurrects it', () => {
    const store = boot()
    store.getState().input({ type: 'receipt', fight_id: FIGHT, version: 6, receipt: { events: [ev('Hit', { victim_is_mob: true, victim_idx: 0, amount: 20, remaining_hp: 0 })] } }, 2_000)
    drain(store, 2_100)
    ...
    store.getState().input({ type: 'poll', fight_id: FIGHT, version: 4, receipt: { events: [ev('MobMoved', { idx: 0, to_cell: encode(7, 4) })] } }, 2_500)
    expect(mob0(store).committed_dead, 'a floored death is permanent against every stale/newer/foreign input').toBe(true)
  ...
  test('trap: detonate@vD then re-lay@vL>vD has zero cross-talk (independent records)', () => {
    ...
    store.getState().input({ type: 'predicted', basis_version: 6, intent_id: 'trapA', actions: [{ kind: 'Cast', caster_is_mob: false, caster_idx: 0, target_cell: TRAP_A, ap_cost: 2 }], beats: [{ kind: 'cast', at: 0, duration: 100, payload: {} }], place_traps: [TRAP_A] }, 1_100)
    // detonate A: a committed fighter lands on it (vD = 7).
    store.getState().input({ type: 'receipt', fight_id: FIGHT, version: 7, receipt: { events: [ev('MobMoved', { idx: 0, to_cell: TRAP_A })] } }, 1_200)
    drain(store, 1_300)
    ...
    store.getState().input({ type: 'predicted', basis_version: 8, intent_id: 'trapB', actions: [{ kind: 'Cast', caster_is_mob: false, caster_idx: 0, target_cell: TRAP_B, ap_cost: 2 }], beats: [{ kind: 'cast', at: 0, duration: 100, payload: {} }], place_traps: [TRAP_B] }, 1_400)
...
describe('§⑤.6 fold-catch-up', () => {
  test('a top-version-only read reconstructs the same terminal board as replaying all K turns (no resurrection)', () => {
    ...
    replay.getState().input({ type: 'init', fight_id: FIGHT, my_key: 'p0', ctx: { my_entity_id: CHAR, beat_ctx: { grid_width: 20 } } })
    replay.getState().input({ type: 'snapshot', fight: fight_object(), version: 5 }, 1_000)
    replay.getState().input({ type: 'receipt', fight_id: FIGHT, version: 6, receipt: { events: [ev('MobMoved', { idx: 0, to_cell: encode(6, 4) })] } }, 1_100)
    replay.getState().input({ type: 'receipt', fight_id: FIGHT, version: 7, receipt: { events: [ev('Hit', { victim_is_mob: true, victim_idx: 0, amount: 15, remaining_hp: 5 })] } }, 1_200)
    replay.getState().input({ type: 'receipt', fight_id: FIGHT, version: 8, receipt: { events: [ev('Hit', { victim_is_mob: true, victim_idx: 0, amount: 5, remaining_hp: 0 })] } }, 1_300)
    ...
    folded.getState().input({ type: 'init', fight_id: FIGHT, my_key: 'p0', ctx: { my_entity_id: CHAR, beat_ctx: { grid_width: 20 } } })
    folded.getState().input({ type: 'snapshot', fight: fight_object(), version: 5 }, 1_000)
    folded.getState().input({ type: 'snapshot', fight: fight_object({ mob_hp: 0, mob_cell: encode(6, 4) }), version: 8 }, 1_100)
    // an out-of-order INTERMEDIATE stale read (mob alive, v7 < the v8 floor) must NOT resurrect it (no flicker).
    folded.getState().input({ type: 'snapshot', fight: fight_object({ mob_hp: 5, mob_cell: encode(6, 4) }), version: 7 }, 1_150)
...
describe('§4 T-B — receipt idempotence · stale no-op · death permanence · provider refusal', () => {
  test('receipt idempotence: a re-delivered receipt does not change committed state', () => {
    const store = boot()
    const r = { type: 'receipt', fight_id: FIGHT, version: 6, receipt: { events: [ev('Hit', { victim_is_mob: true, victim_idx: 0, amount: 5, remaining_hp: 15 })] } }
  ...
  test('provider refusal: a local push while NOT my turn is refused + logged, state unchanged', () => {
    ...
    const before = state_hash(store.getState())
    store.getState().input({ type: 'intent', intent: { kind: 'cast', target_cell: encode(5, 4), damaging: true } }, 2_000)
    expect(store.getState().refused, 'a mismatched-provenance local push is a logged non-event').toMatchObject({ type: 'intent', reason: 'provider' })
