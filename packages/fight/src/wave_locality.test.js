...
describe('receipt wave locality', () => {
  test('only the non-local mob turn enters the paced wave', () => {
    ...
    expect(wave[0].source_id).toBe('mob-1')
    expect(wave.some((turn) => /^player-/.test(turn.source_id))).toBe(false)
    expect(wave.some((turn) => turn.source_id === CHAR)).toBe(false)
  })
})
