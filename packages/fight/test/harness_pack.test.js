...
describe.skipIf(!PACING_ENVELOPES_AVAILABLE)(
  'AOE zone victims — the cross zone resolves correctly on-chain but was never rendered client-side',
  () => {
...
describe.skipIf(!PACING_ENVELOPES_AVAILABLE)('floater timing — §7b E2 (the "at least 1s late" gated complaint)', () => {
...
describe.skipIf(!PACING_ENVELOPES_AVAILABLE)(
  'idle turn commits — a turn with zero staged actions still commits and triggers mob actions',
  () => {
...
describe.skipIf(!PACING_ENVELOPES_AVAILABLE)(
  'turn-bound drafts — a deadline/manual rollover makes a stale auto-flush structurally empty',
  () => {
...
describe.skipIf(!PACING_ENVELOPES_AVAILABLE)(
  'min-turn floor NEVER eats a deadline commit — a commit must not be lost to the 3s toast with 0:01 left',
  () => {
...
describe.skipIf(!PACING_ENVELOPES_AVAILABLE)(
  'strict turn-replay sequencing — the fight-presentation row: full cast sequence in order, none merged, THEN the mob wave',
  () => {
