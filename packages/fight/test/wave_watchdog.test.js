...
import * as fight_store from '../src/store.js'
...
describe('wave watchdog', () => {
  test('an overstaying head is force-acked without a renderer input', () => {
    const store = fight_store.create_fight_store()
    ...
    const [head] = store.getState().wave
    const grace_ms = fight_store.WAVE_ACK_GRACE_MS ?? 6_000
    const t0 = 10_000

    store.getState().input({ type: 'tick' }, t0)
    store.getState().input({ type: 'tick' }, t0 + head.duration + grace_ms + 1)

    expect(store.getState().wave.some((turn) => turn.seq === head.seq)).toBe(false)
  })
})
