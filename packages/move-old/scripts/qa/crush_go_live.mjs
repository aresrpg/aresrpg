...
const createdIdIncl = (r, needle) =>
  (r.objectChanges || []).find((c) => c.type === 'created' && (c.objectType || '').includes(needle))?.objectId
...
async function phase_registry() {
  ...
  for (const [s, t] of RUNES) {
    ...
    const r = await run(
      ...
      (() => {
        ...
        const none_stats = tx.moveCall({
          target: '0x1::option::none',
          typeArguments: [`${L}::item_stats::ItemStatistics`],
        })
        const none_stats2 = tx.moveCall({
          target: '0x1::option::none',
          typeArguments: [`${L}::item_stats::ItemStatistics`],
        })
        const none_eff = tx.moveCall({
          target: '0x1::option::none',
          typeArguments: [`${L}::consumable_effect::ConsumableEffect`],
        })
        ...
        tx.moveCall({
          ...
          arguments: [
            ...
            tx.object(CATALOG),
            tx.pure.string(name),
            tx.pure.string(`rune_${STAT_NAME[s]}_${TIER_NAME[t]}`),
            tx.pure.string('rune'),
            tx.pure.u16(1),
            none_stats,
            none_stats2,
  ...
  for (let i = 0; i < todo.length; i += 8) {
        ...
        for (const [s, t] of batch)
          tx.moveCall({
            ...
            arguments: [
              tx.object(CAP),
              tx.object(BOARD),
              tx.pure.id(ST.runeTemplates[key(s, t)]),
              tx.pure.u8(s),
              tx.pure.u8(t),
              tx.object(VER),
            ],
    ...
    for (const [s, t] of batch) {
      ST.registered[key(s, t)] = true
      ST.registerDigests[key(s, t)] = r.digest
    }
    persist()
...
const statsBlock = (tx) =>
  tx.moveCall({
    target: `${L}::item_stats::new`,
    arguments: FIELDS.map((f) => tx.pure.u16(SHIFT + (GEAR_STATS[f] || 0))),
  })
...
async function phase_gear() {
    ...
    const r = await run(
      ...
      (() => {
        ...
        tx.moveCall({
          ...
          arguments: [tx.object(CAP), tx.object(CREATION), tx.pure.u64(10_000_000n), tx.object(VER)],
        })
        return tx
      })(),
      { ceilingSui: 0.05 }
    )
    ST.gear.setPriceDigest = r.digest
    persist()
  }
  ...
  if (!ST.gear.character) {
    if (!ST.gear.kiosk || !ST.gear.pkcap) {
      ...
      ST.gear.pkcap = (kr.objectChanges || []).find(
        (c) =>
          c.type === 'created' &&
          (c.objectType || '').includes('::personal_kiosk::PersonalKioskCap') &&
          c.owner?.AddressOwner === ADDR
      )?.objectId
    ...
    ST.gear.createCharDigest = cr.digest
    persist()
    console.log(
      `  character=${ST.gear.character?.slice(0, 12)}… kiosk=${ST.gear.kiosk?.slice(0, 12)}… pkcap=${ST.gear.pkcap?.slice(0, 12)}…`
    )
    if (!ST.gear.character || !ST.gear.kiosk || !ST.gear.pkcap)
      throw new Error(`create_char: missing ids in ${cr.digest}`)
  }
  ...
  if (!ST.gear.template) {
    const r = await run(
      ...
      (() => {
        const tx = new Transaction()
        const smin = tx.moveCall({
          target: '0x1::option::some',
          typeArguments: [`${L}::item_stats::ItemStatistics`],
          arguments: [statsBlock(tx)],
        })
        const smax = tx.moveCall({
          target: '0x1::option::some',
          typeArguments: [`${L}::item_stats::ItemStatistics`],
          arguments: [statsBlock(tx)],
        })
        ...
        tx.moveCall({
          ...
          arguments: [
            tx.object(CAP),
            tx.object(CATALOG),
            tx.pure.string('Crush Test Blade'),
    ...
    ST.gear.template = createdId(r, '::item::ItemTemplate')
    ST.gear.templateDigest = r.digest
    persist()
  ...
  if (!ST.gear.sale) {
    const r = await run(
      ...
      (() => {
        ...
        tx.moveCall({
          target: `${L}::shop::create_sale`,
          arguments: [tx.object(CAP), tx.pure.id(ST.gear.template), tx.pure.u64(1_000n), none_supply, tx.object(VER)],
        })
        return tx
      })(),
      { ceilingSui: 0.05 }
    )
    ST.gear.sale = createdIdIncl(r, '::shop::Sale')
    ST.gear.saleDigest = r.digest
    persist()
  ...
  if (!ST.gear.item) {
    const b_tx = buy_ptb({ network })({
      sale_id: ST.gear.sale,
      template_id: ST.gear.template,
      price_mist: 1_000,
      kiosk_id: ST.gear.kiosk,
      personal_kiosk_cap_id: ST.gear.pkcap,
    })
    b_tx.setSenderIfNotSet(ADDR)
    const br = await sui_client.signAndExecuteTransaction({
      signer: keypair,
      transaction: b_tx,
      options: { showEffects: true, showObjectChanges: true },
    })
    await sui_client.waitForTransaction({ digest: br.digest })
    if (br.effects?.status?.status !== 'success')
      throw new Error(`buy FAILED (executed, NOT retrying): ${JSON.stringify(br.effects?.status)} digest=${br.digest}`)
    ST.gear.item = createdId(br, '::item::Item')
    ST.gear.buyDigest = br.digest
    persist()
...
async function phase_crush() {
  ...
  if (ST.crush.digest) {
    console.log(`  already crushed: ${ST.crush.digest} peak=${ST.crush.peakMist}`)
    return
  }
  ...
  if (rune_template_ids.length !== 35 || rune_template_ids.some((x) => !x))
    throw new Error(`need 35 rune templates, have ${rune_template_ids.filter(Boolean).length}`)

  const build_crush = (budget) => {
    const tx = crush_ptb({ network })({
      crush_board_id: BOARD,
      kiosk_id: ST.gear.kiosk,
      personal_kiosk_cap_id: ST.gear.pkcap,
      character_id: ST.gear.character,
      gear_template_id: ST.gear.template,
      gear_item_ids: [ST.gear.item],
      rune_template_ids,
      filler_template_ids: [],
  ...
  if (dr.effects?.status?.status !== 'success')
    throw new Error(
      `crush PRE-FLIGHT dry-run aborted (ZERO gas) — fix before executing: ${JSON.stringify(dr.effects?.status)}`
    )
  const dg = dr.effects.gasUsed
  const dryPeak = Number(dg.computationCost) + Number(dg.storageCost)
  const dryStacks = (dr.objectChanges || []).filter(
    (c) => c.type === 'created' && (c.objectType || '').includes('::item::Item')
  ).length
  console.log(
    `  pre-flight dry-run: success — sample peak=${dryPeak} (${(dryPeak / 1e9).toFixed(4)} SUI), ${dryStacks} stacks`
  )
  ...
  if (dryPeak > 0.56 * CEIL_CRUSH_MIST)
    throw new Error(
      `crush sample peak ${dryPeak} (×1.5=${Math.ceil(dryPeak * 1.5)}) too near ceiling ${CEIL_CRUSH_MIST} — lighten GEAR_STATS (real yield runs ~1 stack heavier than the sample).`
    )
  console.log(
    `  executing for real at ${CRUSH_BUDGET_MIST / 1e6}M cap (≥ ~${(CRUSH_BUDGET_MIST / dryPeak).toFixed(1)}× the sample — absorbs yield variance)`
  )

  const cr = await sui_client.signAndExecuteTransaction({
    signer: keypair,
    transaction: build_crush(CRUSH_BUDGET_MIST),
  ...
  const comp = Number(g.computationCost),
    storage = Number(g.storageCost),
    rebate = Number(g.storageRebate)
  const peak = comp + storage,
  ...
  ST.crush = {
    ...
    netMist: net,
    peakMist: peak,
    budgetMist: CRUSH_BUDGET_MIST,
  }
  const runeStacks = (cr.objectChanges || [])
    .filter((c) => c.type === 'created' && (c.objectType || '').includes('::item::Item'))
    .map((c) => c.objectId)
  ...
  if (st !== 'success')
    throw new Error(
      `CRUSH FAILED (executed, digest=${cr.digest}) — STOP, do not retry (failure-latch). status=${JSON.stringify(cr.effects?.status)}`
    )
  const budget15 = Math.ceil(peak * 1.5)
  console.log(
    `  ⇒ MEASURED_CRUSH_GAS_MIST = ${peak}   (×1.5 per-item budget = ${budget15}, ${(budget15 / 1e9).toFixed(4)} SUI)`
  )
  if (peak * 1.5 > CEIL_CRUSH_MIST)
    console.log(
      `  ⚠ WARNING peak×1.5 (${budget15}) exceeds ceiling ${CEIL_CRUSH_MIST} — reconsider gear before stamping`
    )
}
...
async function phase_verify() {
  ...
  for (const f of df.data) {
    if ((f.name?.type || '').includes('kiosk::Item')) {
      ...
      if ((o.data?.type || '').includes('::item::Item'))
        items.push({
          id: oid,
          template: o.data?.content?.fields?.template,
          category: o.data?.content?.fields?.category,
          amount: o.data?.content?.fields?.amount,
        })
    }
  }
  ...
  for (const it of items) {
    const isRune = runeSet.has(it.template)
    if (isRune) lockedRunes++
    console.log(
      `    ${isRune ? 'RUNE' : it.category} template=${(it.template || '').slice(0, 10)}… amount=${it.amount}`
    )
  }
  ...
  try {
    ...
    if (MEASURED_CRUSH_GAS_MIST == null) {
      console.log('  second-crush: MEASURED_CRUSH_GAS_MIST still null — stamp game.js first, then re-run verify')
      return
    }
    const budget = crush_gas_budget_mist({ items: 1 })
    const rune_template_ids = RUNES.map(([s, t]) => ST.runeTemplates[key(s, t)])
    const c_tx = crush_ptb({ network })({
      crush_board_id: BOARD,
      kiosk_id: ST.gear.kiosk,
      personal_kiosk_cap_id: ST.gear.pkcap,
      character_id: ST.gear.character,
      gear_template_id: ST.gear.template,
      gear_item_ids: ['0x' + '0'.repeat(63) + '1'],
      rune_template_ids,
      filler_template_ids: [],
    })
    ...
    console.log(
      `  second-crush COMPOSES with real budget = ${budget} MIST (${(budget / 1e9).toFixed(4)} SUI) — build OK, not executed`
    )
  } catch (e) {
    console.log(`  second-crush compose error: ${e.message}`)
  }
}
...
if (!run_phase) {
  console.error(`usage: crush_go_live.mjs <registry|gear|crush|verify>`)
  process.exit(1)
}
run_phase()
  .then(() => console.log('\nDONE'))
