...
const request_args = { artisan: id('artisan'), recipe_id: id('r0'), amount_mist: 5000n }
const accept_args = {
  request_id: id('rq0'),
  recipe_id: id('r0'),
...
const redeem_args = { voucher_id: id('v0'), kiosk_id: id('k0'), personal_kiosk_cap_id: id('pk0') }
...
describe('commission v2 builders — refuse loudly when the package is undeployed', () => {
  test('request / accept / execute / cancel / redeem all throw', () => {
    expect(() => commission_request_ptb(undeployed_context)(request_args)).toThrow(/not deployed/)
    expect(() => commission_accept_ptb(undeployed_context)(accept_args)).toThrow(/not deployed/)
    expect(() => commission_execute_ptb(undeployed_context)(execute_args)).toThrow(/not deployed/)
    expect(() => commission_cancel_ptb(undeployed_context)(cancel_args)).toThrow(/not deployed/)
    expect(() => commission_redeem_xp_ptb(undeployed_context)(redeem_args)).toThrow(/not deployed/)
  })
})

describe('commission_request — commission::request, escrow split off gas (no header)', () => {
  test('single target commission::request at LATEST (5 args)', () => {
    const tx = commission_request_ptb(deployed_context)(request_args)
    expect(targets(tx)).toEqual(['commission::request'])
    const req = find_call(tx, 'commission::request')
    expect(req.package).toBe(A.LATEST_PACKAGE_ID)
    expect(req.args).toBe(5)
  })
  test('a ZERO / missing payment still BUILDS client-side (this builder does not pre-validate business floors) — ' +
    'the chain REJECTS it at runtime below the 0.1 SUI floor (EAmountTooLow)', () => {
    expect(() =>
      commission_request_ptb(deployed_context)({ artisan: id('artisan'), recipe_id: id('r0'), amount_mist: 0n }),
    ).not.toThrow()
    expect(() =>
      commission_request_ptb(deployed_context)({ artisan: id('artisan'), recipe_id: id('r0') }),
    ).not.toThrow()
  })
  test('missing artisan or recipe_id refuses', () => {
    expect(() => commission_request_ptb(deployed_context)({ recipe_id: id('r0') })).toThrow(
      /artisan and recipe_id are required/,
    )
    expect(() => commission_request_ptb(deployed_context)({ artisan: id('artisan') })).toThrow(
      /artisan and recipe_id are required/,
    )
  })
})
...
describe('commission_accept — the artisan proves knowledge (commission::accept)', () => {
  test('single target commission::accept at LATEST (7 args)', () => {
    ...
    expect(targets(tx)).toEqual(['commission::accept'])
  ...
  test('any missing required arg refuses', () => {
    expect(() =>
      commission_accept_ptb(deployed_context)({ ...accept_args, character_id: undefined }),
...
describe('commission_execute — runs the craft on the customer kiosk (terminal &Random, no receipt tail)', () => {
  ...
  test('single target commission::execute at LATEST (11 args: + input_item_ids, output, policies, &Random)', () => {
    expect(targets(tx)).toEqual(['commission::execute'])
  ...
  test('empty input_item_ids or missing output_template refuses', () => {
    expect(() =>
      commission_execute_ptb(deployed_context)({ ...execute_args, input_item_ids: [] }),
    ).toThrow(/non-empty array/)
    expect(() =>
      commission_execute_ptb(deployed_context)({ ...execute_args, output_template_id: undefined }),
    ).toThrow(/are required/)
  })
})

describe('commission_cancel — refund (commission::cancel; no config/version gate, no header)', () => {
  test('single target commission::cancel at LATEST (1 arg)', () => {
    const tx = commission_cancel_ptb(deployed_context)(cancel_args)
    expect(targets(tx)).toEqual(['commission::cancel'])
...
describe('commission_redeem_xp — the artisan banks their voucher (commission::redeem_craft_xp)', () => {
  test('single target commission::redeem_craft_xp at LATEST (4 args)', () => {
    ...
    expect(targets(tx)).toEqual(['commission::redeem_craft_xp'])
