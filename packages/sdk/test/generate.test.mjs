...
describe('door parsing (positive controls against the real api.move)', () => {
  ...
  test('every public/entry door of api.move is parsed', () => {
    ...
    expect(doors.map((d) => d.name).sort()).toEqual(declared.sort())
  ...
  test('&Random doors are flagged terminal, others are not', () => {
    const by_name = Object.fromEntries(doors.map((d) => [d.name, d]))
    const terminal = (name) => by_name[name].params.some((p) => p.type === '&Random')
...
describe('the regen-clean tooth (same-commit law)', () => {
  test('committed doors.gen.js is byte-identical to a fresh generation over api.move', async () => {
