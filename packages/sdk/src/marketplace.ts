...
type MarketplaceContext = Readonly<{
  address: string
  kiosk_cap: (kiosk_id?: string) => Promise<KioskOwnerCap | null>
}>
...
export const marketplace_actions = (sdk: Sdk, { address, kiosk_cap }: MarketplaceContext): MarketplaceActions => {
  const cap = async (kiosk?: string): Promise<KioskOwnerCap> => {
    ...
    if (!found) throw new Error('No personal kiosk is available for this marketplace action.')
    return found
  }
  return Object.freeze({
    list: async ({ kind, id, kiosk, price_mist, amount, source_amount, merge_sources = [] }) => {
      ...
      if (amount !== undefined && ![1, 10, 100, 1000].includes(amount))
      ...
      const tx = sdk.tx()
      sdk.with_owner_kiosk(tx, await cap(kiosk), (_kiosk, owner_cap) => {
        for (const source_id of merge_sources) {
          if (source_id === id) throw new Error('A stack cannot merge into itself.')
          merge_stacks_ptb(sdk, tx, { kiosk: _kiosk, cap: owner_cap }, { target_id: id, source_id })
        }
        if (kind === 'item' && amount !== undefined && source_amount !== undefined && amount < source_amount) {
          const lot_id = split_stack_ptb(sdk, tx, { kiosk: _kiosk, cap: owner_cap }, { item_id: id, amount })
          tx.moveCall({
            target: '0x2::kiosk::list',
            typeArguments: [asset_type(sdk, 'item')],
            arguments: [_kiosk, owner_cap, lot_id, tx.pure.u64(price_mist)],
          })
          return
        }
        tx.moveCall({
          ...
          arguments: [_kiosk, owner_cap, tx.pure.id(id), tx.pure.u64(price_mist)],
        })
      })
    ...
    delist: async ({ kind, id, kiosk, existing }) => {
      ...
      sdk.with_owner_kiosk(tx, await cap(kiosk), (_kiosk, owner_cap) => {
        tx.moveCall({
          ...
          typeArguments: [asset_type(sdk, kind)],
          arguments: [_kiosk, owner_cap, tx.pure.id(id)],
        })
        if (kind === 'item' && existing)
          merge_stacks_ptb(sdk, tx, { kiosk: _kiosk, cap: owner_cap }, { target_id: existing, source_id: id })
      })
    ...
    buy: async ({ kind, id, kiosk: seller_kiosk, price_mist, existing, destination_kiosk }) => {
      ...
      const tx = sdk.tx()
      sdk.with_owner_kiosk(tx, await cap(destination_kiosk ?? undefined), (buyer_kiosk, buyer_cap) => {
      ...
      for (const kiosk of kiosks)
        sdk.with_owner_kiosk(tx, await cap(kiosk), (_kiosk, owner_cap) => {
          const coin = tx.moveCall({
            ...
            arguments: [_kiosk, owner_cap, tx.pure.option('u64', null)],
          })
