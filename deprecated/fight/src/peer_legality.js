...
// time over the party transport, is a PREDICTION on my board (source 'intent' → excluded from committed truth,
// retired by the canonical receipt's claim) — so it costs LATENCY, never correctness. But a prediction still
// PAINTS, so an INJECTED illegal action must never reach the eye: this pure gate REDUCES the peer's batch over MY
// committed state with the SAME spatial primitive the local draft/chain use (`bfsPathCost` — the verbatim port of
// combat_grid::bfs_path_cost, los.js), and a batch that could not legally be that fighter's turn is DROPPED and
// FLAGGED, never painted. PURE: no store, no transport, no promises (L-P4) — the store's ONE door calls it.
//
...
//     within the actor's cumulative turn-start MP (the over-budget teleport — the "illegal moves"),
//   · every Cast is by the actor (a peer authoring another's cast is a spoof); a mob-move in a player turn is a spoof.
// WHAT IT DOESN'T (by design): a cast's damage/range needs the private spell corpus the receiver never holds — its
// EFFECTS (Hit/Displaced/Drain/…) ride as presentation and the AUTHORITATIVE receipt is their correctness. The
// source:'intent' exclusion is the STRUCTURAL guarantee underneath: even an un-gated effect never touches committed
// truth, targeting, or victory — the gate is the eye's first line, not the correctness boundary.
...
export const peer_batch_legality = ({ committed, view, actor_key, actions = [], resolve_seat = null }) => {
  ...
  // pool (a stale pre-refill leftover until the turn commits). Unknown budget ⇒ reachability only (never false-flag
  // an honest move for a datum I lack): GRID_CELLS is larger than any real path.
