...
| Guild-perk hooks in core (xp share, guild-gated dungeons) | Core upgrade adds brand doors + DF-based `guild_brand` pin later (new fns + new DF key struct + body-only changes) — the forge/gifting/dungeon `*_brand` twins are the template (config.move:263-320, character_link.move:193,289,373)                         | S1 + brand-twin precedent                     |
...
time. 6v6 today (engine placement bound, config.move:84); bigger formats are engine upgrades,
