...
// scripts/eslint-rules/fp_law.mjs — THE FP-LAW TRIPWIRES (AGENTS.md).
//
// Three lexical rules the mature plugins cannot express without typed linting (the root lint runs
// untyped — see eslint.config.js), each mapped to a law in AGENTS.md:
//
...
const snake_case = {
  meta: {
    ...
    docs: {
      description:
        'dev-chosen bindings are snake_case (AGENTS.md L-N1) — camelCase is a library name, not a declaration choice',
    },
    schema: allow_schema,
    messages: {
      camel:
        '`{{name}}` is camelCase — dev-chosen bindings are snake_case (AGENTS.md L-N1). ' +
...
const no_mutating_methods = {
  meta: {
    ...
    docs: {
      description:
        'no mutation of shared state or parameters (AGENTS.md L-I1/L-I2) — mutate only what THIS function just created',
    },
    schema: allow_schema,
    messages: {
      mutation:
        '`{{text}}` mutates a value this function did not create — "mutation hides change; hidden change ' +
        'manifests chaos" (AGENTS.md L-I1). Copy first (spread / toSorted / toReversed / toSpliced / with), ' +
        'or build the value where it is born.',
      paramMutation:
        "`{{text}}` mutates a function parameter — the caller's value is not yours to change " +
        '(AGENTS.md L-I2). Return a new value instead.',
    },
  },
  create(context) {
    if (allowed_file(context)) return {}
     ...
     *  fold-local accumulation is CONSTRUCTION, AGENTS.md L-I3.) */
...
const no_module_scope_effects = {
  meta: {
    ...
    docs: {
      description:
        'importing a module must be pure (AGENTS.md L-P3) — timers/network/listeners/DOM fire from the app edge, not from module load',
    },
    schema: allow_schema,
    messages: {
      moduleEffect:
        '`{{text}}` runs at module load — importing a module must be pure (AGENTS.md L-P3). ' +
