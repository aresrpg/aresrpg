...
// Per-rule + per-file count ratchet for scripts/entropy-before-validation-gate.sh.
...
const RULE_PREFIX = 'move-entropy-'
...
const normalize_fixture_path = (result_path) =>
  result_path.replace(/^.*fixtures\/entropy_before_validation\/(?:red|green)\//, '')
const counts_of = (semgrep) => {
  const counts = new Map()
  for (const result of semgrep.results.filter((row) => rule_of(row.check_id).startsWith(RULE_PREFIX))) {
...
if (
  ...
  console.error(
    'usage: entropy_before_validation_verdict.mjs --baseline <baseline.json> <semgrep.json> | --write-baseline <baseline.json> <run1.json> <run2.json> <run3.json> | --expect <expected.json> <red|green> <semgrep.json>'
  )
  process.exit(2)
}
const semgrep_path = mode === '--expect' ? fixture_semgrep_path : side_or_semgrep_path
if (!fs.existsSync(baseline_path)) {
  console.error(`ENTROPY-BEFORE-VALIDATION GATE FAILED — baseline missing: ${baseline_path}`)
  process.exit(1)
}
...
if (mode === '--expect') {
  ...
  if (!expected) {
    console.error(`entropy-before-validation self-test: no '${side}' section in ${baseline_path}`)
    process.exit(2)
  }
  ...
  if (mismatches.length > 0) {
    console.error(
      `ENTROPY-BEFORE-VALIDATION SELF-TEST FAILED (${side} fixture) — the matcher no longer has its pinned verdict:`
    )
...
if (regressions.length > 0) {
  console.error('ENTROPY-BEFORE-VALIDATION GATE FAILED — findings above the ratchet baseline:')
  ...
  console.error('Validate deterministic input before consuming &Random; never raise the #1841 baseline.')
  process.exit(1)
}
...
if (improvements.length > 0) {
  ...
  console.log('    bash scripts/entropy-before-validation-gate.sh --write-baseline')
}
