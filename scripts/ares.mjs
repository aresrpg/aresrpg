...
export const unit_test_files = [
  ...
  'test/scripts/corpus_probe.test.mjs', // #2133 — served pointer + blob size/SHA/row proof for the prod corpus probe
  ...
  'scripts/eslint-rules/test_isolation.test.mjs', // the CROSS-FILE TEST-POISON tripwire's RuleTester suite — unrestored top-level namespace spies, the class that made the frontend suite's verdict depend on readdir order
  ...
  'scripts/check-sponsor-config-ssot.test.mjs', // #2197 — the sponsor-config single-home gate's own teeth: each rule driven against a fixture that violates it, so declawing the checker reds instead of passing forever
