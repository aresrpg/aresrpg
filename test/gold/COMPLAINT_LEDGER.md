...
| each spell cast re-arms a 3s end-turn timer (per-cast disease) (number home: SPEC §7b E11)                 | three casts inside the turn add ZERO gating                                                                         | packages/fight/src/turn_pacing.test.js                                    | GATED  |
| end turn is "paused for 3s then available" — the one per-turn floor (number home: SPEC §7b E11)            | the floor is 3s from turn start                                                                                     | packages/fight/src/turn_pacing.test.js                                    | GATED  |
| a sub-3s end-turn must hold to the floor, then commit (number home: SPEC §7b E11)                          | a sub-3s end-turn is HELD until the floor                                                                           | packages/fight/src/turn_pacing.test.js                                    | GATED  |
| mob pushed then "slide back then is pushed again" (poll re-lays board)                                     | convergence — dup poll                                                                                              | packages/fight/src/parity.test.js                                         | GATED  |
| pushed mob shows a phantom "+4 in purple" (bare displacement float)                                        | Cast + Displaced                                                                                                    | packages/fight/src/render_queue_matrix.test.js                            | GATED  |
| "mobs don't seem to attack me and their vfx do not play"                                                   | -mob wave paces at                                                                                                  | packages/fight/src/render_queue_matrix.test.js                            | GATED  |
| mob turn timing — 3s per mob, 6 mobs = 18s (number home: SPEC §7b E10)                                     | six mobs alone                                                                                                      | packages/fight/src/turn_pacing.test.js                                    | GATED  |
...
| invisibility "is not removed when I cast a direct attack"                                                  | my own damaging cast reveals me THIS frame                                                                          | packages/fight/src/invisibility.test.js                                   | GATED  |
...
| every render beat fires from its queue event (renderer × queues)                                           | COVERAGE — every render beat kind                                                                                   | packages/fight/src/render_queue_matrix.test.js                            | GATED  |
